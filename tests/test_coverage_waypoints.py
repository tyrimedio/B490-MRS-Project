import itertools
import importlib.util
import json
import math
import sys
import tempfile
import types
import unittest
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[1]
SUPERVISOR_PATH = (
    REPO_ROOT / "controllers" / "roomba_supervisor" / "roomba_supervisor.py"
)
CONTROLLER_PATH = (
    REPO_ROOT / "controllers" / "roomba_controller" / "roomba_controller.py"
)


def load_supervisor_module():
    sys.modules["controller"] = types.SimpleNamespace(Supervisor=object)
    spec = importlib.util.spec_from_file_location("roomba_supervisor", SUPERVISOR_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def load_controller_module():
    sys.modules["controller"] = types.SimpleNamespace(Robot=object)
    spec = importlib.util.spec_from_file_location("roomba_controller", CONTROLLER_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class CoverageWaypointTests(unittest.TestCase):
    def test_coverage_waypoints_include_final_sweep_row(self):
        supervisor = load_supervisor_module()

        for room, config in supervisor.ROOM_TASKS.items():
            with self.subTest(room=room):
                _, _, _, max_y = config["bounds"]
                sweep_max_y = round(max_y - supervisor.COVERAGE_MARGIN_M, 3)
                waypoints = supervisor.generate_coverage_waypoints(room)
                row_positions = sorted({waypoint[1] for waypoint in waypoints})

                self.assertIn(sweep_max_y, row_positions)
                self.assertEqual(sweep_max_y, row_positions[-1])

    def test_coverage_waypoints_keep_robot_center_away_from_walls(self):
        supervisor = load_supervisor_module()

        for room, config in supervisor.ROOM_TASKS.items():
            with self.subTest(room=room):
                min_x, max_x, min_y, max_y = config["bounds"]
                for x_m, y_m in supervisor.generate_coverage_waypoints(room):
                    self.assertGreaterEqual(
                        x_m - min_x + 1e-9,
                        supervisor.COVERAGE_MARGIN_M,
                    )
                    self.assertGreaterEqual(
                        max_x - x_m + 1e-9,
                        supervisor.COVERAGE_MARGIN_M,
                    )
                    self.assertGreaterEqual(
                        y_m - min_y + 1e-9,
                        supervisor.COVERAGE_MARGIN_M,
                    )
                    self.assertGreaterEqual(
                        max_y - y_m + 1e-9,
                        supervisor.COVERAGE_MARGIN_M,
                    )

    def test_coverage_waypoints_can_split_room_into_lanes(self):
        supervisor = load_supervisor_module()

        full_plan = supervisor.generate_coverage_waypoints("ne_large")
        left_lane = supervisor.generate_coverage_waypoints(
            "ne_large",
            lane_index=0,
            lane_count=2,
        )
        right_lane = supervisor.generate_coverage_waypoints(
            "ne_large",
            lane_index=1,
            lane_count=2,
        )

        full_min_x = min(waypoint[0] for waypoint in full_plan)
        full_max_x = max(waypoint[0] for waypoint in full_plan)
        left_max_x = max(waypoint[0] for waypoint in left_lane)
        right_min_x = min(waypoint[0] for waypoint in right_lane)

        self.assertEqual(full_min_x, min(waypoint[0] for waypoint in left_lane))
        self.assertEqual(full_max_x, max(waypoint[0] for waypoint in right_lane))
        self.assertLessEqual(left_max_x, right_min_x)

    def test_south_room_sweep_starts_near_hub_doorway(self):
        supervisor = load_supervisor_module()

        first_waypoint = supervisor.generate_coverage_waypoints("s_medium")[0]
        _, _, _, max_y = supervisor.ROOM_TASKS["s_medium"]["bounds"]

        self.assertEqual(
            round(max_y - supervisor.COVERAGE_MARGIN_M, 3),
            first_waypoint[1],
        )

    def test_split_lanes_start_on_different_sides(self):
        supervisor = load_supervisor_module()

        left_lane = supervisor.generate_coverage_waypoints(
            "ne_large",
            lane_index=0,
            lane_count=2,
        )
        right_lane = supervisor.generate_coverage_waypoints(
            "ne_large",
            lane_index=1,
            lane_count=2,
        )

        self.assertLess(left_lane[0][0], right_lane[0][0])
        self.assertNotEqual(left_lane[0], right_lane[0])

    def test_coverage_lane_for_robot_uses_stable_room_order(self):
        supervisor = load_supervisor_module()
        room_assignments = {
            "epuck_4": {"room": "ne_large"},
            "epuck_2": {"room": "n_medium"},
            "epuck_1": {"room": "ne_large"},
            "epuck_3": {"room": "ne_large"},
        }

        self.assertEqual(
            (0, 3),
            supervisor.coverage_lane_for_robot("epuck_1", room_assignments),
        )
        self.assertEqual(
            (1, 3),
            supervisor.coverage_lane_for_robot("epuck_3", room_assignments),
        )
        self.assertEqual(
            (2, 3),
            supervisor.coverage_lane_for_robot("epuck_4", room_assignments),
        )

    def test_paused_assignment_is_removed_from_active_room_splits(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large", "helper": True},
        }
        paused_assignments = {}

        paused_assignment = supervisor.pause_robot_assignment(
            "epuck_1",
            room_assignments,
            paused_assignments,
        )

        self.assertEqual({"room": "ne_large"}, paused_assignment)
        self.assertNotIn("epuck_1", room_assignments)
        self.assertEqual(paused_assignment, paused_assignments["epuck_1"])
        self.assertEqual(
            1,
            supervisor.active_robot_count(room_assignments, "ne_large"),
        )
        self.assertEqual(
            (0, 1),
            supervisor.coverage_lane_for_robot("epuck_2", room_assignments),
        )

        emitter = FakeEmitter()
        plans = supervisor.send_room_coverage_plans(
            emitter,
            room_assignments,
            "ne_large",
        )

        self.assertEqual({"epuck_2"}, set(plans))
        self.assertEqual(1, len(emitter.messages))
        plan_command = json.loads(emitter.messages[0])
        self.assertEqual("epuck_2", plan_command["robot"])
        self.assertEqual(0, plan_command["lane_index"])
        self.assertEqual(1, plan_command["lane_count"])

        resumed_assignment = supervisor.resume_robot_assignment(
            "epuck_1",
            room_assignments,
            paused_assignments,
        )

        self.assertEqual(paused_assignment, resumed_assignment)
        self.assertEqual(paused_assignment, room_assignments["epuck_1"])
        self.assertNotIn("epuck_1", paused_assignments)

    def test_operator_redirect_rejects_extra_robot_in_occupied_small_room(self):
        supervisor = load_supervisor_module()
        room_assignments = {
            "epuck_1": {"room": "se_small"},
            "epuck_4": {"room": "ne_large"},
        }

        has_capacity = supervisor.operator_redirect_has_capacity(
            room_assignments,
            "epuck_4",
            "se_small",
            previous_room="ne_large",
        )

        self.assertFalse(has_capacity)

    def test_operator_redirect_counts_paused_small_room_assignment(self):
        supervisor = load_supervisor_module()
        room_assignments = {
            "epuck_4": {"room": "ne_large"},
        }
        paused_assignments = {
            "epuck_1": {"room": "se_small"},
        }

        has_capacity = supervisor.operator_redirect_has_capacity(
            room_assignments,
            "epuck_4",
            "se_small",
            previous_room="ne_large",
            reserved_assignments=paused_assignments,
        )

        self.assertFalse(has_capacity)

    def test_operator_redirect_allows_robot_to_refresh_its_small_room(self):
        supervisor = load_supervisor_module()
        room_assignments = {}
        paused_assignments = {
            "epuck_1": {"room": "se_small"},
        }

        has_capacity = supervisor.operator_redirect_has_capacity(
            room_assignments,
            "epuck_1",
            "se_small",
            previous_room="se_small",
            reserved_assignments=paused_assignments,
        )

        self.assertTrue(has_capacity)

    def test_operator_redirect_rejects_extra_robot_in_full_large_room(self):
        supervisor = load_supervisor_module()
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_4": {"room": "sw_large"},
        }

        has_capacity = supervisor.operator_redirect_has_capacity(
            room_assignments,
            "epuck_4",
            "ne_large",
            previous_room="sw_large",
        )

        self.assertFalse(has_capacity)

    def test_operator_redirect_allows_robot_to_refresh_its_large_room(self):
        supervisor = load_supervisor_module()
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large", "helper": True},
        }

        has_capacity = supervisor.operator_redirect_has_capacity(
            room_assignments,
            "epuck_1",
            "ne_large",
            previous_room="ne_large",
        )

        self.assertTrue(has_capacity)

    def test_coverage_reach_distance_cleans_endpoint_tiles(self):
        controller = load_controller_module()
        supervisor = load_supervisor_module()

        endpoint_to_edge_tile_center_m = (
            supervisor.COVERAGE_MARGIN_M - 0.5 * supervisor.CLEAN_TILE_SIZE_M
        )
        max_reach_distance_m = (
            supervisor.CLEAN_RADIUS_M - endpoint_to_edge_tile_center_m
        )

        self.assertLessEqual(
            controller.COVERAGE_WAYPOINT_REACHED_M,
            max_reach_distance_m,
        )

    def test_coverage_reach_distance_cleans_corner_tiles(self):
        controller = load_controller_module()
        supervisor = load_supervisor_module()

        corner_offset_m = (
            supervisor.COVERAGE_MARGIN_M - 0.5 * supervisor.CLEAN_TILE_SIZE_M
        )
        distance_when_reached_m = (
            (corner_offset_m + controller.COVERAGE_WAYPOINT_REACHED_M) ** 2
            + corner_offset_m ** 2
        ) ** 0.5

        self.assertLessEqual(distance_when_reached_m, supervisor.CLEAN_RADIUS_M)

    def test_odometry_uses_midpoint_heading_for_turning_motion(self):
        controller = load_controller_module()

        left_speed = 1.0
        right_speed = 2.0
        dt_s = 1.0
        x_m, y_m, theta_rad, distance_delta_m, turn_delta_rad = (
            controller.integrate_odometry_pose(
                0.0,
                0.0,
                0.0,
                left_speed,
                right_speed,
                dt_s,
            )
        )

        left_linear_mps = left_speed * controller.WHEEL_RADIUS_M
        right_linear_mps = right_speed * controller.WHEEL_RADIUS_M
        forward_mps = 0.5 * (left_linear_mps + right_linear_mps)
        heading_delta_rad = (
            (right_linear_mps - left_linear_mps)
            / controller.AXLE_LENGTH_M
            * dt_s
        )

        self.assertAlmostEqual(
            forward_mps * controller.math.cos(0.5 * heading_delta_rad),
            x_m,
        )
        self.assertAlmostEqual(
            forward_mps * controller.math.sin(0.5 * heading_delta_rad),
            y_m,
        )
        self.assertAlmostEqual(heading_delta_rad, theta_rad)
        self.assertAlmostEqual(forward_mps, distance_delta_m)
        self.assertAlmostEqual(abs(heading_delta_rad), turn_delta_rad)

    def test_assignment_drive_profile_is_faster_than_launch_near_waypoint(self):
        controller = load_controller_module()

        launch_left, launch_right = controller.drive_toward_target(
            0.0,
            0.0,
            0.0,
            0.35,
            0.0,
        )
        assignment_left, assignment_right = controller.drive_toward_target(
            0.0,
            0.0,
            0.0,
            0.35,
            0.0,
            "assignment",
        )

        launch_average = 0.5 * (launch_left + launch_right)
        assignment_average = 0.5 * (assignment_left + assignment_right)
        self.assertGreater(assignment_average, 1.5 * launch_average)

    def test_coverage_drive_profile_arcs_through_right_angle(self):
        controller = load_controller_module()

        left_speed, right_speed = controller.drive_toward_target(
            0.0,
            0.0,
            0.0,
            0.0,
            0.35,
            "coverage",
        )

        self.assertGreaterEqual(left_speed, 0.0)
        self.assertGreater(right_speed, left_speed)

    def test_assignment_route_waypoints_use_stricter_intermediate_tolerance(self):
        controller = load_controller_module()
        route = [(0.0, 0.0), (0.0, 0.5), (0.0, 1.0)]

        self.assertEqual(
            controller.ASSIGNMENT_ROUTE_WAYPOINT_REACHED_M,
            controller.assignment_waypoint_reached_distance(route, 0),
        )
        self.assertEqual(
            controller.ASSIGNMENT_TARGET_REACHED_M,
            controller.assignment_waypoint_reached_distance(route, 2),
        )

    def test_lidar_geometry_uses_forward_offset_and_beam_fan(self):
        controller = load_controller_module()

        origin_x_m, origin_y_m = controller.lidar_sensor_origin(1.0, 2.0, 0.0)
        rays = controller.lidar_free_ray_endpoints(1.0, 2.0, 0.0, 0.5)
        hit_x_m, hit_y_m = controller.lidar_wall_hit_point(1.0, 2.0, 0.0, 0.5)

        self.assertAlmostEqual(1.0 + controller.LIDAR_FORWARD_OFFSET_M, origin_x_m)
        self.assertAlmostEqual(2.0, origin_y_m)
        self.assertEqual(3, len(rays))
        self.assertTrue(all(ray[0] == origin_x_m for ray in rays))
        self.assertTrue(all(ray[1] == origin_y_m for ray in rays))
        self.assertAlmostEqual(origin_x_m + 0.5, hit_x_m)
        self.assertAlmostEqual(origin_y_m, hit_y_m)
        self.assertLess(rays[0][3], origin_y_m)
        self.assertGreater(rays[2][3], origin_y_m)

    def test_pose_confidence_decays_with_odometry_distance_and_turning(self):
        controller = load_controller_module()

        confident = controller.pose_confidence_from_odometry(0.0, 0.0)
        stale = controller.pose_confidence_from_odometry(
            controller.POSE_CONFIDENCE_DISTANCE_DECAY_M,
            controller.POSE_CONFIDENCE_TURN_DECAY_RAD,
        )

        self.assertEqual(1.0, confident)
        self.assertEqual(controller.MIN_POSE_CONFIDENCE, stale)

    def test_robot_holds_after_launch_until_assignment_arrives(self):
        controller = load_controller_module()

        self.assertTrue(
            controller.should_hold_for_assignment(
                launch_finished=True,
                launch_timed_out=False,
                assigned_target=None,
            )
        )
        self.assertTrue(
            controller.should_hold_for_assignment(
                launch_finished=False,
                launch_timed_out=True,
                assigned_target=None,
            )
        )
        self.assertFalse(
            controller.should_hold_for_assignment(
                launch_finished=True,
                launch_timed_out=False,
                assigned_target=(1.0, 1.0),
            )
        )

    def test_robot_holds_after_task_completion(self):
        controller = load_controller_module()

        self.assertTrue(
            controller.should_hold_after_task(
                assigned_target=(1.0, 1.0),
                assignment_target_reached=True,
                coverage_complete=True,
            )
        )

    def test_robot_phase_names_explain_current_behavior(self):
        controller = load_controller_module()

        self.assertEqual(
            "launching",
            controller.robot_phase(False, False, None, False, False, "sweep"),
        )
        self.assertEqual(
            "waiting_for_assignment",
            controller.robot_phase(True, False, None, False, False, "sweep"),
        )
        self.assertEqual(
            "driving_to_room",
            controller.robot_phase(True, False, (1.0, 1.0), False, False, "sweep"),
        )
        self.assertEqual(
            "waiting_for_route",
            controller.robot_phase(
                True,
                False,
                (1.0, 1.0),
                False,
                False,
                "sweep",
                assignment_wait_steps_remaining=4,
            ),
        )
        self.assertEqual(
            "sweeping",
            controller.robot_phase(True, False, (1.0, 1.0), True, False, "sweep"),
        )
        self.assertEqual(
            "cleanup",
            controller.robot_phase(True, False, (1.0, 1.0), True, False, "cleanup"),
        )
        self.assertEqual(
            "holding_after_task",
            controller.robot_phase(True, False, (1.0, 1.0), True, True, "cleanup"),
        )
        self.assertFalse(
            controller.should_hold_after_task(
                assigned_target=(1.0, 1.0),
                assignment_target_reached=False,
                coverage_complete=True,
            )
        )

    def test_launch_waypoints_stage_robots_in_central_hub(self):
        controller = load_controller_module()

        for robot_name, waypoints in controller.ROBOT_LAUNCH_WAYPOINTS.items():
            with self.subTest(robot=robot_name):
                self.assertEqual(1, len(waypoints))
                x_m, y_m = waypoints[0]
                self.assertLessEqual(abs(x_m), 0.5)
                self.assertLessEqual(abs(y_m), 0.5)

    def test_actual_robot_pose_uses_webots_translation(self):
        supervisor_module = load_supervisor_module()

        class FakeTranslationField:
            def getSFVec3f(self):
                return [1.25, -2.5, 0.0]

        class FakeRotationField:
            def getSFRotation(self):
                return [0.0, 0.0, 1.0, 1.5]

        class FakeRobotNode:
            def getField(self, field_name):
                if field_name == "translation":
                    return FakeTranslationField()
                if field_name == "rotation":
                    return FakeRotationField()
                return None

        class FakeSupervisor:
            def getFromDef(self, def_name):
                if def_name == "EPUCK_3":
                    return FakeRobotNode()
                return None

        pose = supervisor_module.get_actual_robot_pose(FakeSupervisor(), "epuck_3")

        self.assertEqual({"x_m": 1.25, "y_m": -2.5, "theta_rad": 1.5}, pose)
        self.assertIsNone(
            supervisor_module.get_actual_robot_pose(FakeSupervisor(), "missing")
        )

    def test_dashboard_soft_reset_restores_robot_webots_poses(self):
        supervisor = load_supervisor_module()

        class FakeField:
            def __init__(self):
                self.value = None

            def setSFVec3f(self, value):
                self.value = value

            def setSFRotation(self, value):
                self.value = value

        class FakeRobotNode:
            def __init__(self):
                self.translation = FakeField()
                self.rotation = FakeField()
                self.reset_count = 0

            def getField(self, field_name):
                if field_name == "translation":
                    return self.translation
                if field_name == "rotation":
                    return self.rotation
                return None

            def resetPhysics(self):
                self.reset_count += 1

        class FakeSupervisor:
            def __init__(self):
                self.nodes = {
                    "EPUCK_1": FakeRobotNode(),
                    "EPUCK_3": FakeRobotNode(),
                }

            def getFromDef(self, def_name):
                return self.nodes.get(def_name)

        fake_supervisor = FakeSupervisor()

        reset_count = supervisor.reset_webots_robot_poses(fake_supervisor)

        self.assertEqual(2, reset_count)
        epuck_1 = fake_supervisor.nodes["EPUCK_1"]
        epuck_3 = fake_supervisor.nodes["EPUCK_3"]
        self.assertEqual([-1.5, 0.0, supervisor.ROBOT_START_Z_M], epuck_1.translation.value)
        self.assertEqual([0.0, 0.0, 1.0, 0.5 * math.pi], epuck_1.rotation.value)
        self.assertEqual([0.5, 0.0, supervisor.ROBOT_START_Z_M], epuck_3.translation.value)
        self.assertEqual([0.0, 0.0, 1.0, -0.5 * math.pi], epuck_3.rotation.value)
        self.assertEqual(1, epuck_1.reset_count)
        self.assertEqual(1, epuck_3.reset_count)

    def test_dashboard_soft_reset_sends_robot_controller_reset_command(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()

        supervisor.send_sim_reset_command(emitter)

        self.assertEqual([{"type": "sim_reset", "robot": "all"}], emitter.messages)

    def test_cleaning_path_interpolation_covers_status_update_gaps(self):
        supervisor = load_supervisor_module()
        start_pose = {"x_m": 0.0, "y_m": 0.0}
        end_pose = {"x_m": 0.24, "y_m": 0.0}

        points = supervisor.interpolate_cleaning_path(start_pose, end_pose)
        largest_gap_m = max(
            (
                ((points[index][0] - points[index - 1][0]) ** 2
                 + (points[index][1] - points[index - 1][1]) ** 2) ** 0.5
                for index in range(1, len(points))
            ),
            default=0.0,
        )

        self.assertEqual((0.0, 0.0), points[0])
        self.assertEqual((0.24, 0.0), points[-1])
        self.assertLessEqual(
            largest_gap_m,
            supervisor.CLEAN_TRAIL_SAMPLE_SPACING_M + 1e-9,
        )

    def test_cleaning_marks_only_after_robot_enters_assigned_room(self):
        supervisor = load_supervisor_module()
        room = "nw_small"
        min_x, max_x, min_y, max_y = supervisor.ROOM_TASKS[room]["bounds"]
        inside_pose = {
            "x_m": 0.5 * (min_x + max_x),
            "y_m": 0.5 * (min_y + max_y),
        }
        outside_pose = {"x_m": 0.0, "y_m": 0.0}

        self.assertFalse(supervisor.robot_can_mark_cleaning(outside_pose, room))
        self.assertTrue(supervisor.robot_can_mark_cleaning(inside_pose, room))
        self.assertFalse(supervisor.robot_can_mark_cleaning(None, room))
        self.assertFalse(
            supervisor.robot_can_mark_cleaning(inside_pose, "not_a_room")
        )

    def test_room_progress_percent_uses_dirty_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {"nw_small": 4}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 1, 0),
        }

        self.assertEqual(2, overlay.cleaned_tile_count("nw_small"))
        self.assertEqual(50.0, overlay.room_progress_percent("nw_small"))

    def test_room_reached_coverage_goal_requires_no_dirty_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {"nw_small": 20}
        overlay.dirty_tiles = {("nw_small", 0, 0)}

        self.assertFalse(
            supervisor.room_reached_coverage_goal(overlay, "nw_small")
        )

        overlay.dirty_tiles = set()
        self.assertTrue(
            supervisor.room_reached_coverage_goal(overlay, "nw_small")
        )

    def test_dirty_tile_centers_targets_remaining_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 1, 0),
            ("n_medium", 0, 0),
        }

        centers = overlay.dirty_tile_centers("nw_small")

        self.assertEqual([[-2.875, 0.875], [-2.625, 0.875]], centers)

    def test_dirty_tile_centers_align_to_floor_checkerboard(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.dirty_tiles = {
            ("ne_large", 0, 0),
            ("s_medium", 0, 0),
        }

        self.assertEqual([[0.875, 0.875]], overlay.dirty_tile_centers("ne_large"))
        self.assertEqual([[-0.625, -2.875]], overlay.dirty_tile_centers("s_medium"))

    def test_cleanup_claims_give_robots_different_dirty_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 1, 0),
            ("nw_small", 2, 0),
            ("nw_small", 3, 0),
        }

        first_plan = overlay.claim_dirty_tile_centers(
            "nw_small",
            "epuck_1",
            {"x_m": -2.9, "y_m": 0.9},
            max_tiles=2,
        )
        second_plan = overlay.claim_dirty_tile_centers(
            "nw_small",
            "epuck_2",
            {"x_m": -2.1, "y_m": 0.9},
            max_tiles=2,
        )

        self.assertEqual(2, len(first_plan))
        self.assertEqual(2, len(second_plan))
        self.assertEqual(
            set(),
            set(map(tuple, first_plan)) & set(map(tuple, second_plan)),
        )
        self.assertEqual(
            {
                ("nw_small", 0, 0): "epuck_1",
                ("nw_small", 1, 0): "epuck_1",
                ("nw_small", 2, 0): "epuck_2",
                ("nw_small", 3, 0): "epuck_2",
            },
            overlay.tile_claims,
        )

    def test_cleanup_claims_prefer_nearby_dirty_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 1, 0),
            ("nw_small", 2, 0),
        }

        plan = overlay.claim_dirty_tile_centers(
            "nw_small",
            "epuck_1",
            {"x_m": -2.62, "y_m": 0.88},
            max_tiles=1,
        )

        self.assertEqual([[-2.625, 0.875]], plan)
        self.assertEqual({("nw_small", 1, 0): "epuck_1"}, overlay.tile_claims)

    def test_cleanup_claims_can_prioritize_wall_and_corner_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 3, 3),
        }

        plan = overlay.claim_dirty_tile_centers(
            "nw_small",
            "epuck_1",
            {"x_m": -2.12, "y_m": 1.62},
            max_tiles=1,
            prefer_edges=True,
        )

        self.assertEqual([[-2.875, 0.875]], plan)
        self.assertEqual({("nw_small", 0, 0): "epuck_1"}, overlay.tile_claims)

    def test_cleanup_trigger_reason_starts_final_cleanup_before_sweep_end(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {"nw_small": 100}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("nw_small", 1, 0),
            ("nw_small", 2, 0),
            ("nw_small", 3, 0),
        }

        reason = supervisor.cleanup_trigger_reason(
            overlay,
            "nw_small",
            coverage_done_for_room=False,
            room_stalled=False,
        )

        self.assertEqual("final 4 dirty tile(s) remain", reason)

    def test_cleanup_trigger_reason_starts_single_robot_batch_cleanup(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {"ne_large": 100}
        overlay.dirty_tiles = {
            ("ne_large", col_index, 0)
            for col_index in range(supervisor.CLEANUP_TARGETS_PER_ROBOT)
        }

        reason = supervisor.cleanup_trigger_reason(
            overlay,
            "ne_large",
            coverage_done_for_room=False,
            room_stalled=False,
        )

        self.assertEqual("8 dirty tile(s) fit one cleanup robot", reason)

    def test_cleanup_trigger_reason_starts_stalled_late_cleanup(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {"nw_small": 100}
        overlay.dirty_tiles = {
            ("nw_small", col_index, 0)
            for col_index in range(7)
        }

        reason = supervisor.cleanup_trigger_reason(
            overlay,
            "nw_small",
            coverage_done_for_room=False,
            room_stalled=True,
        )

        self.assertEqual("room stalled at 93.0% clean", reason)

    def test_cleanup_pass_resends_stale_identical_plan(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {("nw_small", 0, 0): "epuck_1"}
        overlay.dirty_tiles = {("nw_small", 0, 0)}
        cleanup_waypoint = supervisor.cleanup_waypoint_for_tile_center(
            "nw_small",
            [-2.875, 0.875],
        )
        cleanup_plan_signatures = {
            "epuck_1": ("nw_small", (tuple(cleanup_waypoint),))
        }
        cleanup_plan_steps = {"epuck_1": 10}

        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "nw_small",
            {"x_m": -2.8, "y_m": 0.9},
            cleanup_plan_signatures,
            cleanup_plan_steps,
            10 + supervisor.CLEANUP_RESEND_STEPS,
            "room stalled at 99.0% clean",
        )

        self.assertEqual([cleanup_waypoint], plan)
        self.assertEqual(1, len(emitter.messages))
        command = json.loads(emitter.messages[0])
        self.assertEqual("cleanup", command["plan_kind"])

    def test_cleanup_plan_skips_operator_no_go_dirty_tiles(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("nw_small", 0, 0)}
        no_go_zone = {
            "name": "blocked tile",
            "bounds": [-2.95, -2.8, 0.8, 0.95],
        }

        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "nw_small",
            {"x_m": -2.8, "y_m": 0.9},
            {},
            {},
            20,
            "final dirty tile remains",
            [no_go_zone],
        )

        self.assertIsNone(plan)
        self.assertEqual([], emitter.messages)
        self.assertEqual({}, overlay.tile_claims)

    def test_cleanup_plan_releases_claim_when_no_go_detour_is_unreachable(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("n_medium", 5, 5)}
        no_go_zones = [
            {"name": "top block", "bounds": [-0.2, 0.2, 0.2, 0.3]},
            {"name": "bottom block", "bounds": [-0.2, 0.2, -0.3, -0.2]},
            {"name": "left block", "bounds": [-0.3, -0.2, -0.3, 0.3]},
            {"name": "right block", "bounds": [0.2, 0.3, -0.3, 0.3]},
        ]

        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "n_medium",
            {"x_m": 0.0, "y_m": 0.0},
            {},
            {},
            20,
            "final dirty tile remains",
            no_go_zones,
        )

        self.assertIsNone(plan)
        self.assertEqual([], emitter.messages)
        self.assertEqual({}, overlay.tile_claims)

        second_plan = overlay.claim_dirty_tile_centers(
            "n_medium",
            "epuck_2",
            {"x_m": -0.4, "y_m": 1.5},
            no_go_zones=no_go_zones,
        )

        self.assertGreater(len(second_plan), 0)
        self.assertEqual({("n_medium", 5, 5): "epuck_2"}, overlay.tile_claims)

    def test_cleanup_plan_releases_claims_omitted_by_partial_no_go_detour(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("n_medium", 4, 2),
        }
        no_go_zones = [
            {"name": "top block", "bounds": [-0.7, -0.1, 1.7, 1.8]},
            {"name": "bottom block", "bounds": [-0.7, -0.1, 1.2, 1.3]},
            {"name": "left block", "bounds": [-0.8, -0.7, 1.2, 1.8]},
            {"name": "right block", "bounds": [-0.1, 0.0, 1.2, 1.8]},
        ]

        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "n_medium",
            {"x_m": -0.4, "y_m": 1.5},
            {},
            {},
            20,
            "final dirty tiles remain",
            no_go_zones,
        )

        self.assertEqual([[-0.375, 1.375]], plan)
        self.assertEqual({("n_medium", 4, 2): "epuck_1"}, overlay.tile_claims)

        second_plan = overlay.claim_dirty_tile_centers(
            "n_medium",
            "epuck_2",
            {"x_m": -1.2, "y_m": 0.9},
            no_go_zones=no_go_zones,
        )

        self.assertEqual([[-1.375, 0.875]], second_plan)
        self.assertEqual(
            {
                ("n_medium", 0, 0): "epuck_2",
                ("n_medium", 4, 2): "epuck_1",
            },
            overlay.tile_claims,
        )

    def test_operator_no_go_dirty_tile_does_not_block_room_completion(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"nw_small": 1}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("nw_small", 0, 0)}
        no_go_zone = {
            "name": "blocked final tile",
            "bounds": [-2.95, -2.8, 0.8, 0.95],
        }

        self.assertIsNone(
            supervisor.cleanup_trigger_reason(
                overlay,
                "nw_small",
                coverage_done_for_room=True,
                no_go_zones=[no_go_zone],
            )
        )
        self.assertFalse(
            supervisor.room_reached_coverage_goal(overlay, "nw_small")
        )
        self.assertTrue(
            supervisor.room_reached_coverage_goal(
                overlay,
                "nw_small",
                [no_go_zone],
            )
        )
        self.assertEqual(
            100.0,
            overlay.room_progress_percent("nw_small", [no_go_zone]),
        )

    def test_operator_blocked_room_entry_excludes_room_dirt_and_cleanup(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"n_medium": 1}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("n_medium", 5, 5)}
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        self.assertTrue(
            supervisor.room_entry_blocked_by_no_go_zones(
                "n_medium",
                [no_go_zone],
            )
        )
        self.assertEqual(0, overlay.dirty_tile_count("n_medium", [no_go_zone]))
        self.assertTrue(
            supervisor.room_reached_coverage_goal(
                overlay,
                "n_medium",
                [no_go_zone],
            )
        )
        self.assertIsNone(
            supervisor.cleanup_trigger_reason(
                overlay,
                "n_medium",
                coverage_done_for_room=True,
                no_go_zones=[no_go_zone],
            )
        )

        emitter = FakeEmitter()
        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "n_medium",
            {"x_m": 0.0, "y_m": 0.0},
            {},
            {},
            20,
            "sweep complete",
            [no_go_zone],
        )

        self.assertIsNone(plan)
        self.assertEqual([], emitter.messages)
        self.assertEqual({}, overlay.tile_claims)

    def test_operator_blocked_room_entry_still_counts_dirt_for_inside_robot(self):
        supervisor = load_supervisor_module()

        class FakeSupervisor:
            def getFromDef(self, def_name):
                return None

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"n_medium": 1}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("n_medium", 5, 5)}
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": -0.4, "y_m": 1.5}},
        }

        self.assertTrue(
            supervisor.assigned_robot_inside_room(
                FakeSupervisor(),
                latest_robot_status,
                room_assignments,
                "n_medium",
            )
        )
        self.assertGreater(
            len(
                supervisor.send_room_coverage_plans(
                    FakeEmitter(),
                    room_assignments,
                    "n_medium",
                    no_go_zones=[no_go_zone],
                    latest_robot_status=latest_robot_status,
                )["epuck_1"]
            ),
            0,
        )
        self.assertEqual(0, overlay.dirty_tile_count("n_medium", [no_go_zone]))
        self.assertEqual(
            1,
            overlay.dirty_tile_count(
                "n_medium",
                [no_go_zone],
                exclude_entry_blocked=False,
            ),
        )
        self.assertFalse(
            supervisor.room_reached_coverage_goal(
                overlay,
                "n_medium",
                [no_go_zone],
                exclude_entry_blocked=False,
            )
        )
        self.assertIsNotNone(
            supervisor.cleanup_trigger_reason(
                overlay,
                "n_medium",
                coverage_done_for_room=True,
                no_go_zones=[no_go_zone],
                exclude_entry_blocked=False,
            )
        )

        emitter = FakeEmitter()
        plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_1",
            "n_medium",
            {"x_m": -0.4, "y_m": 1.5},
            {},
            {},
            20,
            "sweep complete",
            [no_go_zone],
            entry_blocked_is_reachable=True,
        )

        self.assertIsNotNone(plan)
        self.assertGreater(len(plan), 0)
        self.assertEqual("cleanup", emitter.messages[0]["plan_kind"])

    def test_no_go_change_reopens_completed_room_when_dirt_becomes_reachable(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {room: 1 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("nw_small", 0, 0)}
        no_go_zone = {
            "name": "blocked final tile",
            "bounds": [-2.95, -2.8, 0.8, 0.95],
        }
        completed_rooms = set(supervisor.ROOM_TASKS)
        completed_robot_rooms = {
            "epuck_1": "nw_small",
            "epuck_2": "n_medium",
        }

        self.assertTrue(
            supervisor.room_reached_coverage_goal(
                overlay,
                "nw_small",
                [no_go_zone],
            )
        )
        self.assertEqual(
            [],
            supervisor.reopen_completed_rooms_with_reachable_dirt(
                completed_rooms,
                completed_robot_rooms,
                overlay,
                [no_go_zone],
            ),
        )

        reopened_rooms = supervisor.reopen_completed_rooms_with_reachable_dirt(
            completed_rooms,
            completed_robot_rooms,
            overlay,
            [],
        )

        self.assertEqual(["nw_small"], reopened_rooms)
        self.assertNotIn("nw_small", completed_rooms)
        self.assertEqual({"epuck_2": "n_medium"}, completed_robot_rooms)

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            {"epuck_1": {"room": "s_medium"}},
            completed_rooms,
            overlay,
            latest_robot_status={"epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}}},
            no_go_zones=[],
        )

        self.assertEqual("nw_small", next_room)

    def test_no_go_reopened_room_assigns_idle_robot(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.enabled = False
        overlay.active_rooms = set()
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {room: 1 for room in supervisor.ROOM_TASKS}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("nw_small", 0, 0)}
        completed_rooms = set(supervisor.ROOM_TASKS)
        completed_robot_rooms = {}
        supervisor.reopen_completed_rooms_with_reachable_dirt(
            completed_rooms,
            completed_robot_rooms,
            overlay,
            [],
        )
        room_assignments = {
            robot_name: None for robot_name in supervisor.EXPECTED_ROBOTS
        }
        coverage_plans = {}

        assigned_rooms = supervisor.assign_idle_robots_to_unfinished_rooms(
            emitter,
            room_assignments,
            completed_rooms,
            overlay,
            {"epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}}},
            set(),
            coverage_plans,
            {},
            {},
            {},
            {},
            no_go_zones=[],
        )

        self.assertEqual({"epuck_1": "nw_small"}, assigned_rooms)
        self.assertEqual("nw_small", room_assignments["epuck_1"]["room"])
        self.assertIn("epuck_1", coverage_plans)
        self.assertEqual("task_assignment", emitter.messages[0]["type"])
        self.assertEqual("coverage_plan", emitter.messages[1]["type"])

    def test_idle_robot_waits_when_only_occupied_small_room_remains(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.enabled = False
        overlay.active_rooms = set()
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(8)
        }
        room_assignments = {
            "epuck_1": None,
            "epuck_2": {"room": "se_small"},
        }
        coverage_plans = {}
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}},
            "epuck_2": {"pose": {"x_m": 2.0, "y_m": -2.0}},
        }

        assigned_rooms = supervisor.assign_idle_robots_to_unfinished_rooms(
            emitter,
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"se_small"},
            overlay,
            latest_robot_status,
            set(),
            coverage_plans,
            {},
            {},
            {},
            {},
            no_go_zones=[],
        )

        self.assertEqual({}, assigned_rooms)
        self.assertIsNone(room_assignments["epuck_1"])
        self.assertEqual({}, coverage_plans)

    def test_no_go_change_assigns_idle_robot_to_newly_reachable_room(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        robot_statuses = {
            robot: {"pose": {"x_m": 0.0, "y_m": 0.0}}
            for robot in supervisor.EXPECTED_ROBOTS
        }
        blocking_no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }
        allocator = supervisor.TaskAllocator(
            supervisor.ROOM_TASKS,
            supervisor.MRTA_DISTANCE_WEIGHT,
            supervisor.MRTA_AREA_WEIGHT,
        )
        initial_assignments = allocator.assign(
            robot_statuses,
            no_go_zones=[blocking_no_go_zone],
        )
        room_assignments = {
            robot: initial_assignments.get(robot)
            for robot in supervisor.EXPECTED_ROBOTS
        }
        idle_robots = [
            robot for robot, assignment in room_assignments.items()
            if assignment is None
        ]
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.enabled = False
        overlay.active_rooms = set()
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {room: 1 for room in supervisor.ROOM_TASKS}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {("n_medium", 0, 0)}
        coverage_plans = {}
        emitter = FakeEmitter()

        reopened_rooms, assigned_rooms = supervisor.handle_operator_no_go_change(
            emitter,
            room_assignments,
            set(),
            {},
            overlay,
            robot_statuses,
            set(),
            coverage_plans,
            {},
            {},
            {},
            {},
            no_go_zones=[],
        )

        self.assertEqual(1, len(idle_robots))
        self.assertEqual([], reopened_rooms)
        self.assertEqual({idle_robots[0]: "n_medium"}, assigned_rooms)
        self.assertEqual("n_medium", room_assignments[idle_robots[0]]["room"])
        self.assertIn(idle_robots[0], coverage_plans)
        self.assertEqual("task_assignment", emitter.messages[0]["type"])
        self.assertEqual("coverage_plan", emitter.messages[1]["type"])

    def test_no_go_removal_reopens_completed_room_and_shows_overlay(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        class FakeOverlay:
            def __init__(self):
                self.rooms = supervisor.ROOM_TASKS
                self.room_tile_counts = {
                    room: 5 for room in supervisor.ROOM_TASKS
                }
                self.dirty_tiles = {
                    ("se_small", col_index, 0)
                    for col_index in range(5)
                }
                self.tile_claims = {}
                self.shown_rooms = []

            def show_dirty_room(self, room):
                self.shown_rooms.append(room)

            def dirty_tile_count(
                self,
                room,
                no_go_zones=None,
                exclude_entry_blocked=True,
            ):
                if (
                    exclude_entry_blocked
                    and supervisor.room_entry_blocked_by_no_go_zones(room, no_go_zones)
                ):
                    return 0
                return sum(1 for tile in self.dirty_tiles if tile[0] == room)

            def room_progress_percent(
                self,
                room,
                no_go_zones=None,
                exclude_entry_blocked=True,
            ):
                return 100.0 * (
                    self.room_tile_counts[room]
                    - self.dirty_tile_count(room, no_go_zones, exclude_entry_blocked)
                ) / self.room_tile_counts[room]

            def release_robot_claims(self, robot_name):
                self.tile_claims = {
                    tile: owner
                    for tile, owner in self.tile_claims.items()
                    if owner != robot_name
                }

        emitter = FakeEmitter()
        overlay = FakeOverlay()
        completed_rooms = set(supervisor.ROOM_TASKS)
        completed_robot_rooms = {"epuck_4": "se_small"}
        room_assignments = {
            robot: None for robot in supervisor.EXPECTED_ROBOTS
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}},
        }
        coverage_plans = {}

        reopened_rooms, assigned_rooms = supervisor.handle_operator_no_go_change(
            emitter,
            room_assignments,
            completed_rooms,
            completed_robot_rooms,
            overlay,
            latest_robot_status,
            set(),
            coverage_plans,
            {},
            {},
            {},
            {},
            no_go_zones=[],
        )

        self.assertEqual(["se_small"], reopened_rooms)
        self.assertNotIn("se_small", completed_rooms)
        self.assertEqual({}, completed_robot_rooms)
        self.assertIn("se_small", overlay.shown_rooms)
        self.assertEqual({"epuck_1": "se_small"}, assigned_rooms)
        self.assertEqual("se_small", room_assignments["epuck_1"]["room"])

    def test_cleaning_a_tile_releases_its_claim(self):
        supervisor = load_supervisor_module()

        class FakeField:
            def setSFColor(self, value):
                self.value = value

            def setSFFloat(self, value):
                self.value = value

        class FakeAppearance:
            def getField(self, field_name):
                return FakeField()

        tile_key = ("nw_small", 0, 0)
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.enabled = True
        overlay.active_rooms = {"nw_small"}
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.dirty_tiles = {tile_key}
        overlay.tile_claims = {tile_key: "epuck_1"}
        overlay.tile_appearances = {tile_key: FakeAppearance()}

        cleaned_count = overlay.mark_clean_near("nw_small", -2.875, 0.875)

        self.assertEqual(1, cleaned_count)
        self.assertEqual(set(), overlay.dirty_tiles)
        self.assertEqual({}, overlay.tile_claims)

    def test_releasing_robot_claims_keeps_other_robot_claims(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.tile_claims = {
            ("nw_small", 0, 0): "epuck_1",
            ("nw_small", 1, 0): "epuck_2",
            ("n_medium", 0, 0): "epuck_1",
        }

        overlay.release_robot_claims("epuck_1")

        self.assertEqual({("nw_small", 1, 0): "epuck_2"}, overlay.tile_claims)

    def test_room_progress_snapshot_reports_every_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("n_medium", 1, 0),
            ("ne_large", 0, 0),
            ("ne_large", 1, 0),
            ("ne_large", 2, 0),
        }

        snapshot = supervisor.room_progress_snapshot(overlay)

        expected = {room: 100.0 for room in supervisor.ROOM_TASKS}
        expected["n_medium"] = 80.0
        expected["ne_large"] = 70.0
        self.assertEqual(expected, snapshot)

        formatted = supervisor.format_room_progress(snapshot)
        self.assertIn("n_medium=80.0%", formatted)
        self.assertIn("ne_large=70.0%", formatted)
        self.assertIn("nw_small=100.0%", formatted)

    def test_overall_coverage_percent_weights_every_room_tile(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("n_medium", 1, 0),
            ("ne_large", 0, 0),
        }

        coverage_percent = supervisor.overall_coverage_percent(overlay)

        self.assertAlmostEqual(95.0, coverage_percent)

    def test_evaluation_metrics_track_target_time_and_collisions(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("n_medium", 1, 0),
            ("ne_large", 0, 0),
            ("ne_large", 1, 0),
        }
        metrics = supervisor.EvaluationMetrics(
            single_robot_baseline_time_s=100.0,
            single_robot_baseline_metric="coverage_target_time_s",
        )

        metrics.mark_started(10, 2.0)
        self.assertEqual([], metrics.update_progress(overlay, 20, 5.0))

        overlay.dirty_tiles = {("n_medium", 0, 0), ("ne_large", 0, 0)}
        progress_events = metrics.update_progress(overlay, 30, 12.0)
        self.assertEqual(1, len(progress_events))
        self.assertIn("Coverage target reached", progress_events[0])

        first_collision = metrics.update_collisions(
            {
                "epuck_1": {"x_m": 0.0, "y_m": 0.0},
                "epuck_2": {"x_m": 0.03, "y_m": 0.0},
            },
            31,
            12.1,
        )
        repeated_collision = metrics.update_collisions(
            {
                "epuck_1": {"x_m": 0.0, "y_m": 0.0},
                "epuck_2": {"x_m": 0.04, "y_m": 0.0},
            },
            32,
            12.2,
        )

        snapshot = metrics.snapshot(40, 14.0)

        self.assertEqual(1, len(first_collision))
        self.assertEqual([], repeated_collision)
        self.assertEqual(10.0, snapshot["coverage_target_time_s"])
        self.assertEqual(1, snapshot["inter_robot_collisions"])
        self.assertFalse(snapshot["collision_free"])
        self.assertEqual(90.0, snapshot["time_reduction_percent"])

    def test_complete_baseline_waits_for_complete_cleaning_time(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("ne_large", 0, 0),
        }
        metrics = supervisor.EvaluationMetrics(
            single_robot_baseline_time_s=100.0,
            single_robot_baseline_metric="complete_cleaning_time_s",
        )

        metrics.mark_started(0, 0.0)
        metrics.update_progress(overlay, 10, 10.0)
        target_snapshot = metrics.snapshot(10, 10.0)

        overlay.dirty_tiles = set()
        metrics.update_progress(overlay, 40, 40.0)
        complete_snapshot = metrics.snapshot(40, 40.0)

        self.assertEqual(10.0, target_snapshot["coverage_target_time_s"])
        self.assertIsNone(target_snapshot["complete_cleaning_time_s"])
        self.assertIsNone(target_snapshot["time_reduction_percent"])
        self.assertEqual(40.0, complete_snapshot["complete_cleaning_time_s"])
        self.assertEqual(60.0, complete_snapshot["time_reduction_percent"])

    def test_complete_time_reopens_when_coverage_drops_below_full(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = set()
        metrics = supervisor.EvaluationMetrics()

        metrics.mark_started(0, 0.0)
        metrics.update_progress(overlay, 10, 10.0)
        complete_snapshot = metrics.snapshot(10, 10.0)

        overlay.dirty_tiles = {("n_medium", 0, 0)}
        metrics.update_progress(overlay, 20, 20.0)
        reopened_snapshot = metrics.snapshot(20, 20.0)

        overlay.dirty_tiles = set()
        metrics.update_progress(overlay, 30, 30.0)
        recomplete_snapshot = metrics.snapshot(30, 30.0)

        self.assertEqual(10.0, complete_snapshot["complete_cleaning_time_s"])
        self.assertIsNone(reopened_snapshot["complete_cleaning_time_s"])
        self.assertEqual(20.0, reopened_snapshot["elapsed_cleaning_time_s"])
        self.assertEqual(30.0, recomplete_snapshot["complete_cleaning_time_s"])

    def test_coverage_target_reopens_when_progress_drops_below_target(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("ne_large", 0, 0),
        }
        metrics = supervisor.EvaluationMetrics(
            single_robot_baseline_time_s=100.0,
            single_robot_baseline_metric="coverage_target_time_s",
        )

        metrics.mark_started(0, 0.0)
        metrics.update_progress(overlay, 10, 10.0)
        target_snapshot = metrics.snapshot(10, 10.0)

        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("n_medium", 1, 0),
            ("ne_large", 0, 0),
            ("ne_large", 1, 0),
        }
        metrics.update_progress(overlay, 20, 20.0)
        reopened_snapshot = metrics.snapshot(20, 20.0)

        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("ne_large", 0, 0),
        }
        metrics.update_progress(overlay, 30, 30.0)
        retargeted_snapshot = metrics.snapshot(30, 30.0)

        self.assertEqual(10.0, target_snapshot["coverage_target_time_s"])
        self.assertTrue(target_snapshot["coverage_target_met"])
        self.assertIsNone(reopened_snapshot["coverage_target_time_s"])
        self.assertFalse(reopened_snapshot["coverage_target_met"])
        self.assertIsNone(reopened_snapshot["time_reduction_percent"])
        self.assertEqual(30.0, retargeted_snapshot["coverage_target_time_s"])
        self.assertEqual(70.0, retargeted_snapshot["time_reduction_percent"])

    def test_single_robot_baseline_loader_accepts_metrics_json(self):
        supervisor = load_supervisor_module()

        with tempfile.TemporaryDirectory() as temp_dir:
            baseline_path = Path(temp_dir) / "single_robot_baseline.json"
            baseline_path.write_text(
                json.dumps({"cleaning_time_s": 123.456}),
                encoding="utf-8",
            )
            object_baseline = supervisor.load_single_robot_baseline(baseline_path)

            baseline_path.write_text("90.5", encoding="utf-8")
            numeric_baseline = supervisor.load_single_robot_baseline(baseline_path)

            baseline_path.write_text(
                json.dumps({"complete_cleaning_time_s": 88.0}),
                encoding="utf-8",
            )
            complete_baseline = supervisor.load_single_robot_baseline(baseline_path)

            baseline_path.write_text(
                json.dumps({"cleaning_time_s": -1.0}),
                encoding="utf-8",
            )
            invalid_baseline = supervisor.load_single_robot_baseline(baseline_path)

        self.assertEqual(
            {"time_s": 123.456, "metric": "elapsed_cleaning_time_s"},
            object_baseline,
        )
        self.assertEqual(
            {"time_s": 90.5, "metric": "elapsed_cleaning_time_s"},
            numeric_baseline,
        )
        self.assertEqual(
            {"time_s": 88.0, "metric": "complete_cleaning_time_s"},
            complete_baseline,
        )
        self.assertIsNone(invalid_baseline)

    def test_operator_state_reports_robot_rooms_and_progress(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("n_medium", 0, 0), ("n_medium", 1, 0)}
        controls = types.SimpleNamespace(
            priority_zones=[
                {
                    "name": "north focus",
                    "room": "n_medium",
                    "bounds": supervisor.ROOM_TASKS["n_medium"]["bounds"],
                }
            ],
            no_go_zones=[
                {
                    "name": "north block",
                    "room": "n_medium",
                    "bounds": supervisor.ROOM_TASKS["n_medium"]["bounds"],
                }
            ],
            pending_redirects=lambda: [],
            pending_sim_reset_request=lambda: None,
            last_error="",
        )
        traffic = types.SimpleNamespace(
            summary=lambda: {
                "planned_distance_m": 4.25,
                "waiting_steps": 12,
                "doorway_conflicts": 1,
            }
        )
        status = {
            "pose": {"x_m": -0.5, "y_m": 1.2, "theta_rad": 0.1},
            "phase": "sweeping",
            "coverage": {
                "room": "n_medium",
                "waypoint_index": 3,
                "waypoint_count": 10,
                "complete": False,
                "plan_kind": "sweep",
            },
            "assignment_route": {"waiting_steps_remaining": 0},
            "assignment_target_reached": True,
        }

        class FakeSupervisor:
            def getFromDef(self, def_name):
                return None

        state = supervisor.build_operator_state(
            FakeSupervisor(),
            45,
            {"epuck_1": status},
            {"epuck_1": {"room": "n_medium", "helper": False}},
            overlay,
            controls,
            set(),
            set(),
            traffic,
        )

        epuck_1 = state["robots"][0]
        n_medium = next(room for room in state["rooms"] if room["name"] == "n_medium")
        nw_small = next(room for room in state["rooms"] if room["name"] == "nw_small")
        self.assertEqual(45, state["step"])
        self.assertEqual("sweeping", epuck_1["phase"])
        self.assertEqual("n_medium", epuck_1["room"])
        self.assertEqual({"x_m": -0.5, "y_m": 1.2, "theta_rad": 0.1}, epuck_1["pose"])
        self.assertEqual(80.0, n_medium["progress_percent"])
        self.assertEqual(["E1"], n_medium["assigned_robots"])
        self.assertEqual(["north focus"], n_medium["priority_zones"])
        self.assertEqual(["north block"], n_medium["no_go_zones"])
        self.assertEqual([], nw_small["no_go_zones"])
        self.assertEqual(1, state["traffic"]["doorway_conflicts"])
        self.assertIn("coverage_percent", state["metrics"])

    def test_operator_state_counts_blocked_room_dirt_when_robot_is_inside(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"n_medium": 1}
        overlay.dirty_tiles = {("n_medium", 5, 5)}
        controls = types.SimpleNamespace(
            priority_zones=[],
            no_go_zones=[
                {
                    "name": "North doorways",
                    "bounds": [-2.55, 2.15, 0.45, 1.05],
                }
            ],
            pending_redirects=lambda: [],
            pending_sim_reset_request=lambda: None,
            last_error="",
        )
        traffic = types.SimpleNamespace(
            summary=lambda: {
                "planned_distance_m": 0.0,
                "waiting_steps": 0,
                "doorway_conflicts": 0,
            }
        )
        status = {
            "pose": {"x_m": -0.4, "y_m": 1.5, "theta_rad": 0.0},
            "phase": "sweeping",
            "coverage": {
                "room": "n_medium",
                "waypoint_index": 1,
                "waypoint_count": 12,
                "complete": False,
                "plan_kind": "sweep",
            },
            "assignment_route": {"waiting_steps_remaining": 0},
            "assignment_target_reached": True,
        }

        class FakeSupervisor:
            def getFromDef(self, def_name):
                return None

        state = supervisor.build_operator_state(
            FakeSupervisor(),
            80,
            {"epuck_1": status},
            {"epuck_1": {"room": "n_medium", "helper": False}},
            overlay,
            controls,
            set(),
            set(),
            traffic,
        )

        n_medium = next(room for room in state["rooms"] if room["name"] == "n_medium")
        self.assertEqual(0.0, n_medium["progress_percent"])
        self.assertEqual(1, n_medium["dirty_tiles"])
        self.assertEqual(["E1"], n_medium["assigned_robots"])
        self.assertEqual(["North doorways"], n_medium["no_go_zones"])

    def test_progress_monitor_counts_blocked_room_dirt_when_robot_is_inside(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"n_medium": 1}
        overlay.dirty_tiles = {("n_medium", 0, 3)}
        no_go_zones = [
            {
                "name": "North doorways",
                "bounds": [-2.55, 2.15, 0.45, 1.05],
            }
        ]
        latest_robot_status = {
            "epuck_1": {
                "pose": {"x_m": -0.4, "y_m": 1.5, "theta_rad": 0.0},
                "assignment_target_reached": True,
            }
        }
        room_assignments = {"epuck_1": {"room": "n_medium"}}

        class FakeSupervisor:
            def getFromDef(self, def_name):
                return None

        self.assertEqual(
            100.0,
            supervisor.room_progress_snapshot(overlay, no_go_zones)["n_medium"],
        )
        entry_blocked_reachable_rooms = {
            room
            for room in supervisor.ROOM_TASKS
            if supervisor.assigned_robot_inside_room(
                FakeSupervisor(),
                latest_robot_status,
                room_assignments,
                room,
            )
        }
        progress_snapshot = supervisor.room_progress_snapshot(
            overlay,
            no_go_zones,
            entry_blocked_reachable_rooms=entry_blocked_reachable_rooms,
        )
        progress_monitors = {
            "n_medium": {
                "progress": 0.0,
                "step": 0,
                "ready": True,
            }
        }

        stalled_rooms = supervisor.update_room_progress_monitors(
            progress_monitors,
            progress_snapshot,
            room_assignments,
            set(),
            supervisor.ROOM_PROGRESS_STALL_STEPS,
            {"n_medium"},
        )

        self.assertEqual(0.0, progress_snapshot["n_medium"])
        self.assertEqual(["n_medium"], stalled_rooms)

    def test_bounds_overlap_requires_shared_area_not_touching_edges(self):
        supervisor = load_supervisor_module()

        self.assertFalse(
            supervisor.bounds_overlap(
                supervisor.ROOM_TASKS["n_medium"]["bounds"],
                supervisor.ROOM_TASKS["nw_small"]["bounds"],
            )
        )
        self.assertTrue(
            supervisor.bounds_overlap(
                [-1.4, -1.0, 1.0, 2.0],
                supervisor.ROOM_TASKS["n_medium"]["bounds"],
            )
        )

    def test_robot_motion_monitor_flags_robot_that_should_move_but_does_not(self):
        supervisor = load_supervisor_module()
        monitors = {}
        assignment = {"room": "n_medium"}
        status = {
            "launch": {"complete": True, "timed_out": False},
            "assignment_target_reached": False,
            "coverage": {"room": "n_medium", "complete": False},
            "motion": {"front_blocked": True},
        }
        pose = {"x_m": 0.0, "y_m": 0.0}

        stuck, _ = supervisor.update_robot_motion_monitor(
            monitors,
            "epuck_1",
            status,
            pose,
            assignment,
            0,
        )
        self.assertFalse(stuck)

        stuck, reason = supervisor.update_robot_motion_monitor(
            monitors,
            "epuck_1",
            status,
            pose,
            assignment,
            supervisor.ROBOT_STUCK_WINDOW_STEPS,
        )

        self.assertTrue(stuck)
        self.assertEqual("blocked by nearby obstacle", reason)

    def test_route_start_delay_does_not_count_as_stuck_motion(self):
        supervisor = load_supervisor_module()
        status = {
            "launch": {"complete": True, "timed_out": False},
            "assignment_target_reached": False,
            "assignment_route": {"waiting_steps_remaining": 12},
            "coverage": {"room": "n_medium", "complete": False},
        }

        self.assertFalse(
            supervisor.robot_should_be_moving(status, {"room": "n_medium"})
        )

    def test_robot_motion_monitor_resets_when_robot_moves_enough(self):
        supervisor = load_supervisor_module()
        monitors = {}
        assignment = {"room": "n_medium"}
        status = {
            "launch": {"complete": True, "timed_out": False},
            "assignment_target_reached": True,
            "coverage": {"room": "n_medium", "complete": False},
            "motion": {},
        }

        supervisor.update_robot_motion_monitor(
            monitors,
            "epuck_1",
            status,
            {"x_m": 0.0, "y_m": 0.0},
            assignment,
            0,
        )
        stuck, _ = supervisor.update_robot_motion_monitor(
            monitors,
            "epuck_1",
            status,
            {"x_m": supervisor.ROBOT_STUCK_MIN_MOVE_M, "y_m": 0.0},
            assignment,
            supervisor.ROBOT_STUCK_WINDOW_STEPS,
        )

        self.assertFalse(stuck)
        self.assertEqual(
            supervisor.ROBOT_STUCK_WINDOW_STEPS,
            monitors["epuck_1"]["anchor_step"],
        )

    def test_room_progress_monitor_flags_active_room_without_cleaning_gain(self):
        supervisor = load_supervisor_module()
        monitors = {}
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        progress = {room: 100.0 for room in supervisor.ROOM_TASKS}
        progress["n_medium"] = 40.0

        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            0,
        )
        self.assertEqual([], stalled_rooms)

        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            supervisor.ROOM_PROGRESS_STALL_STEPS,
        )

        self.assertEqual(["n_medium"], stalled_rooms)

    def test_room_progress_monitor_waits_until_robot_can_clean_room(self):
        supervisor = load_supervisor_module()
        monitors = {}
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        progress = {room: 100.0 for room in supervisor.ROOM_TASKS}
        progress["n_medium"] = 40.0

        supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            0,
            progress_ready_rooms=set(),
        )
        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            supervisor.ROOM_PROGRESS_STALL_STEPS,
            progress_ready_rooms=set(),
        )

        self.assertEqual([], stalled_rooms)

        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            2 * supervisor.ROOM_PROGRESS_STALL_STEPS,
            progress_ready_rooms={"n_medium"},
        )
        self.assertEqual([], stalled_rooms)

        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            3 * supervisor.ROOM_PROGRESS_STALL_STEPS,
            progress_ready_rooms={"n_medium"},
        )
        self.assertEqual(["n_medium"], stalled_rooms)

    def test_room_progress_monitor_resets_after_cleaning_gain(self):
        supervisor = load_supervisor_module()
        monitors = {}
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        progress = {room: 100.0 for room in supervisor.ROOM_TASKS}
        progress["n_medium"] = 40.0
        supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            0,
        )

        progress["n_medium"] = 40.2
        stalled_rooms = supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            supervisor.ROOM_PROGRESS_STALL_STEPS,
        )

        self.assertEqual([], stalled_rooms)
        self.assertEqual(
            supervisor.ROOM_PROGRESS_STALL_STEPS,
            monitors["n_medium"]["step"],
        )

    def test_room_progress_monitor_tracks_recent_cleaning_rate(self):
        supervisor = load_supervisor_module()
        monitors = {}
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        progress = {room: 100.0 for room in supervisor.ROOM_TASKS}
        progress["n_medium"] = 40.0

        supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            0,
        )

        progress["n_medium"] = 41.0
        supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            100,
        )
        self.assertAlmostEqual(
            10.0,
            supervisor.room_progress_rate(monitors, "n_medium"),
        )

        supervisor.update_room_progress_monitors(
            monitors,
            progress,
            room_assignments,
            set(),
            100 + supervisor.ROOM_PROGRESS_RATE_WINDOW_STEPS,
        )
        self.assertEqual(0.0, supervisor.room_progress_rate(monitors, "n_medium"))

    def test_stalled_room_redirect_prefers_idle_robot(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("n_medium", 0, 0)}
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": None,
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "se_small"},
        }

        robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
        )

        self.assertEqual("epuck_2", robot)

    def test_stalled_room_redirect_prefers_closest_idle_robot(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("n_medium", 0, 0)}
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": None,
            "epuck_3": None,
            "epuck_4": {"room": "se_small"},
        }
        latest_robot_status = {
            "epuck_2": {"pose": {"x_m": 2.6, "y_m": -2.6}},
            "epuck_3": {"pose": {"x_m": -0.2, "y_m": 0.2}},
        }

        robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual("epuck_3", robot)

    def test_stalled_room_redirect_ignores_idle_robots_that_cannot_reach_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("n_medium", 0, 0)}
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": None,
            "epuck_3": None,
            "epuck_4": {"room": "se_small"},
        }
        latest_robot_status = {
            "epuck_2": {"pose": {"x_m": 0.0, "y_m": 0.0}},
            "epuck_3": {"pose": {"x_m": 0.1, "y_m": 0.0}},
        }
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
            latest_robot_status=latest_robot_status,
            no_go_zones=[no_go_zone],
        )

        self.assertIsNone(robot)

    def test_stalled_room_redirect_can_reuse_helper_from_supported_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("ne_large", 0, 0),
        }
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "ne_large", "helper": True},
        }

        robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
        )

        self.assertEqual("epuck_2", robot)

    def test_stalled_room_redirect_keeps_helper_in_supported_dirty_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(8)
        } | {
            ("se_small", col_index, 0)
            for col_index in range(8)
        }
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "se_small"},
        }

        robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
        )

        self.assertIsNone(robot)

    def test_stalled_room_redirect_skips_robot_already_redirected_this_pass(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 10 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", 0, 0),
            ("ne_large", 0, 0),
            ("sw_large", 0, 0),
        }
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "ne_large", "helper": True},
        }

        first_robot = supervisor.select_robot_for_stalled_room(
            "n_medium",
            room_assignments,
            overlay,
        )
        self.assertEqual("epuck_2", first_robot)

        room_assignments[first_robot] = {"room": "n_medium", "helper": True}
        second_robot = supervisor.select_robot_for_stalled_room(
            "sw_large",
            room_assignments,
            overlay,
            excluded_robots={first_robot},
        )

        self.assertIsNone(second_robot)

    def test_stalled_small_room_redirect_stays_on_stalled_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(12)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(8)
        }
        room_assignments = {
            "epuck_1": {"room": "se_small"},
            "epuck_2": {"room": "se_small", "helper": True},
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "s_medium"},
        }

        redirect_room = supervisor.select_redirect_room_for_stalled_progress(
            "se_small",
            room_assignments,
            {"nw_small", "n_medium", "sw_large", "s_medium"},
            overlay,
        )

        self.assertEqual("se_small", redirect_room)

    def test_stalled_small_room_does_not_pull_extra_helper(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(12)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(8)
        }
        room_assignments = {
            "epuck_1": {"room": "se_small"},
            "epuck_2": None,
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "ne_large", "helper": True},
        }

        robot = supervisor.select_robot_for_stalled_room(
            "se_small",
            room_assignments,
            overlay,
        )

        self.assertIsNone(robot)

    def test_reassignment_selects_least_supported_unfinished_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", col_index, 0)
            for col_index in range(12)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(5)
        } | {
            ("se_small", col_index, 0)
            for col_index in range(8)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "ne_large"},
            "epuck_4": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small"},
            overlay,
        )

        self.assertEqual("n_medium", next_room)

    def test_reassignment_keeps_unstaffed_rooms_ahead_of_nearby_supported_rooms(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("s_medium", col_index, 0)
            for col_index in range(70)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(20)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "s_medium"},
            "epuck_3": {"room": "se_small"},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.3, "y_m": -0.7}},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "sw_large", "se_small"},
            overlay,
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_balances_helpers_before_progress(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", col_index, 0)
            for col_index in range(12)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(6)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "n_medium", "helper": True},
            "epuck_4": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small"},
            overlay,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_prefers_large_room_below_target_over_small_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(14)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(4)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "se_small"},
            "epuck_3": {"room": "se_small", "helper": True},
            "epuck_4": {"room": "ne_large"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "sw_large", "s_medium"},
            overlay,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_prefers_slow_progress_room_when_support_matches(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("ne_large", col_index, 0)
            for col_index in range(40)
        } | {
            ("sw_large", col_index, 0)
            for col_index in range(40)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "ne_large"},
            "epuck_3": {"room": "sw_large"},
        }
        progress_monitors = {
            "ne_large": {"rate_percent_per_1000_steps": 8.0},
            "sw_large": {"rate_percent_per_1000_steps": 0.0},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "s_medium", "se_small"},
            overlay,
            progress_monitors=progress_monitors,
        )

        self.assertEqual("sw_large", next_room)

    def test_reassignment_uses_route_cost_when_need_is_similar(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", col_index, 0)
            for col_index in range(40)
        } | {
            ("s_medium", col_index, 0)
            for col_index in range(40)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "s_medium"},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.2, "y_m": -0.2}},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "ne_large", "sw_large", "se_small"},
            overlay,
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual("s_medium", next_room)

    def test_reassignment_skips_operator_unreachable_rooms(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", col_index, 0)
            for col_index in range(40)
        } | {
            ("s_medium", col_index, 0)
            for col_index in range(40)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "sw_large"},
            "epuck_3": {"room": "se_small"},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}},
        }
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "ne_large", "sw_large", "se_small"},
            overlay,
            latest_robot_status=latest_robot_status,
            no_go_zones=[no_go_zone],
        )

        self.assertEqual("s_medium", next_room)

    def test_reassignment_uses_operator_priority_as_tiebreaker(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("n_medium", col_index, 0)
            for col_index in range(40)
        } | {
            ("s_medium", col_index, 0)
            for col_index in range(40)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "s_medium"},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.2, "y_m": -0.2}},
        }
        priority_zones = [
            supervisor.normalize_operator_zone(
                {"name": "north priority", "room": "n_medium", "weight": 3.0},
                "priority",
            )
        ]

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "ne_large", "sw_large", "se_small"},
            overlay,
            latest_robot_status=latest_robot_status,
            priority_zones=priority_zones,
        )

        self.assertEqual("n_medium", next_room)

    def test_operator_priority_does_not_override_unstaffed_room_support(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("s_medium", col_index, 0)
            for col_index in range(70)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(20)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "s_medium"},
            "epuck_3": {"room": "se_small"},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": 0.3, "y_m": -0.7}},
        }
        priority_zones = [
            supervisor.normalize_operator_zone(
                {"name": "south priority", "room": "s_medium", "weight": 10.0},
                "priority",
            )
        ]

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "sw_large", "se_small"},
            overlay,
            latest_robot_status=latest_robot_status,
            priority_zones=priority_zones,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_can_join_room_that_already_has_helper(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("ne_large", col_index, 0)
            for col_index in range(12)
        } | {
            ("sw_large", col_index, 0)
            for col_index in range(10)
        }
        room_assignments = {
            "epuck_1": {"room": "ne_large", "helper": True},
            "epuck_2": {"room": "sw_large", "helper": True},
            "epuck_3": {"room": "s_medium"},
            "epuck_4": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_3",
            room_assignments,
            {"nw_small", "n_medium", "s_medium", "se_small"},
            overlay,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_does_not_add_third_robot_to_large_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("ne_large", col_index, 0)
            for col_index in range(12)
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "ne_large"},
            "epuck_3": {"room": "ne_large", "helper": True},
            "epuck_4": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "sw_large", "s_medium", "se_small"},
            overlay,
        )

        self.assertIsNone(next_room)

    def test_reassignment_prefers_understaffed_large_room_before_small_helper(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(12)
        } | {
            ("ne_large", col_index, 0)
            for col_index in range(6)
        }
        room_assignments = {
            "epuck_1": {"room": "se_small"},
            "epuck_2": {"room": "n_medium"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_2",
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"se_small", "ne_large"},
            overlay,
        )

        self.assertEqual("ne_large", next_room)

    def test_reassignment_skips_staffed_room_with_only_final_tiles(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 80 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("ne_large", 0, 0)}
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "n_medium"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_2",
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"ne_large"},
            overlay,
        )

        self.assertIsNone(next_room)

    def test_late_reassignment_does_not_crowd_occupied_small_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(12)
        }
        room_assignments = {
            "epuck_1": {"room": "se_small"},
            "epuck_2": {"room": "n_medium"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_2",
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"se_small"},
            overlay,
        )

        self.assertIsNone(next_room)

    def test_reassignment_counts_paused_small_room_assignment(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("se_small", col_index, 0)
            for col_index in range(12)
        }
        room_assignments = {
            "epuck_2": {"room": "n_medium"},
        }
        paused_assignments = {
            "epuck_1": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_2",
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"se_small"},
            overlay,
            reserved_assignments=paused_assignments,
        )

        self.assertIsNone(next_room)

    def test_reassignment_returns_none_when_all_rooms_are_complete(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = set()
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "s_medium"},
            "epuck_4": {"room": "se_small"},
        }

        next_room = supervisor.select_reassignment_room(
            "epuck_1",
            room_assignments,
            set(supervisor.ROOM_TASKS),
            overlay,
        )

        self.assertIsNone(next_room)

    def test_stuck_recovery_retries_current_unfinished_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("nw_small", 0, 0),
            ("ne_large", 0, 0),
        }
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "s_medium"},
            "epuck_4": {"room": "se_small"},
        }

        recovery_room = supervisor.select_stuck_recovery_room(
            "epuck_1",
            room_assignments,
            {"n_medium", "s_medium", "se_small"},
            overlay,
        )

        self.assertEqual("nw_small", recovery_room)

    def test_stuck_recovery_counts_blocked_room_dirt_when_robot_is_inside(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {room: 100 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("n_medium", 0, 4)}
        room_assignments = {"epuck_1": {"room": "n_medium"}}
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        recovery_room = supervisor.select_stuck_recovery_room(
            "epuck_1",
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"n_medium"},
            overlay,
            no_go_zones=[no_go_zone],
            robot_inside_room=True,
        )

        self.assertEqual("n_medium", recovery_room)

    def test_repeated_stuck_robot_can_switch_from_supported_room(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {
            ("ne_large", 0, 0),
            ("n_medium", 0, 0),
        }
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large"},
            "epuck_3": {"room": "s_medium"},
            "epuck_4": {"room": "se_small"},
        }

        recovery_room = supervisor.select_stuck_recovery_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "s_medium", "se_small"},
            overlay,
            recovery_count=supervisor.STUCK_ROUTE_RETRY_LIMIT + 1,
        )

        self.assertEqual("n_medium", recovery_room)

    def test_stuck_recovery_can_reassign_after_current_room_is_complete(self):
        supervisor = load_supervisor_module()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.room_tile_counts = {room: 20 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {("ne_large", 0, 0)}
        room_assignments = {
            "epuck_1": {"room": "nw_small"},
            "epuck_2": {"room": "n_medium"},
            "epuck_3": {"room": "s_medium"},
            "epuck_4": {"room": "se_small"},
        }

        recovery_room = supervisor.select_stuck_recovery_room(
            "epuck_1",
            room_assignments,
            {"nw_small", "n_medium", "s_medium", "se_small"},
            overlay,
        )

        self.assertEqual("ne_large", recovery_room)

    def test_build_assignment_cost_uses_current_robot_pose(self):
        supervisor = load_supervisor_module()
        center_x_m, center_y_m = supervisor.ROOM_TASKS["ne_large"]["center"]
        status = {"pose": {"x_m": center_x_m, "y_m": center_y_m}}

        assignment = supervisor.build_assignment(status, "ne_large", helper=True)

        self.assertEqual("ne_large", assignment["room"])
        self.assertEqual((center_x_m, center_y_m), assignment["target"])
        self.assertTrue(assignment["helper"])
        expected_cost = round(
            supervisor.MRTA_AREA_WEIGHT * supervisor.ROOM_TASKS["ne_large"]["area_m2"],
            3,
        )
        self.assertAlmostEqual(expected_cost, assignment["cost"])
        self.assertEqual(
            supervisor.generate_assignment_route(status["pose"], "ne_large"),
            assignment["route"],
        )

    def test_assignment_route_lines_up_before_room_to_room_doorways(self):
        supervisor = load_supervisor_module()
        pose = {"x_m": -2.85, "y_m": 2.2}

        route = supervisor.generate_assignment_route(pose, "ne_large")

        self.assertEqual([-2.25, 1.2], route[0])
        self.assertEqual([-2.25, 0.93], route[1])
        self.assertEqual([-2.25, 0.57], route[2])
        self.assertEqual([-2.25, 0.3], route[3])
        self.assertNotIn(supervisor.HUB_ROUTE_WAYPOINT, route)
        self.assertEqual([1.85, 0.3], route[-5])
        self.assertEqual([1.85, 0.57], route[-4])
        self.assertEqual([1.85, 0.93], route[-3])
        self.assertEqual([1.85, 1.2], route[-2])
        self.assertEqual([1.85, 1.875], route[-1])

    def test_assignment_route_from_hub_skips_shared_center_waypoint(self):
        supervisor = load_supervisor_module()
        pose = {"x_m": -0.3, "y_m": 0.1}

        route = supervisor.generate_assignment_route(pose, "n_medium")

        self.assertNotIn(supervisor.HUB_ROUTE_WAYPOINT, route)
        self.assertEqual([-0.4, 0.3], route[0])
        self.assertEqual([-0.4, 0.57], route[1])
        self.assertEqual([-0.4, 0.93], route[2])
        self.assertEqual([-0.4, 1.2], route[3])
        self.assertEqual([-0.4, 1.875], route[4])

    def test_assignment_route_can_end_at_lane_start(self):
        supervisor = load_supervisor_module()
        pose = {"x_m": -0.3, "y_m": 0.1}
        lane_start = supervisor.generate_coverage_waypoints("n_medium")[0]

        route = supervisor.generate_assignment_route(
            pose,
            "n_medium",
            final_waypoint=lane_start,
        )

        self.assertEqual(lane_start, route[-1])

    def test_operator_control_config_parses_pause_zones_and_redirects(self):
        supervisor = load_supervisor_module()
        parsed = supervisor.parse_operator_control_config(
            {
                "paused_robots": ["epuck_2", "ghost"],
                "priority_zones": [
                    {"name": "north priority", "room": "n_medium", "weight": 2.0}
                ],
                "no_go_zones": [
                    {"name": "hub block", "bounds": [0.4, -0.4, 0.25, -0.25]}
                ],
                "redirects": [
                    {
                        "id": "send-3-north",
                        "robot": "epuck_3",
                        "room": "ne_large",
                    },
                    {"robot": "ghost", "room": "ne_large"},
                ],
                "sim_reset_request": {
                    "id": "reset-demo",
                    "requested_at": 123.4567,
                },
            }
        )

        self.assertEqual({"epuck_2"}, parsed["paused_robots"])
        self.assertEqual("north priority", parsed["priority_zones"][0]["name"])
        self.assertEqual(
            list(supervisor.ROOM_TASKS["n_medium"]["bounds"]),
            parsed["priority_zones"][0]["bounds"],
        )
        self.assertEqual([-0.4, 0.4, -0.25, 0.25], parsed["no_go_zones"][0]["bounds"])
        self.assertEqual(
            [{"id": "send-3-north", "robot": "epuck_3", "room": "ne_large"}],
            parsed["redirects"],
        )
        self.assertEqual(
            {"id": "reset-demo", "requested_at": 123.457},
            parsed["sim_reset_request"],
        )

    def test_reset_operator_control_file_writes_empty_dashboard_config(self):
        supervisor = load_supervisor_module()

        with tempfile.TemporaryDirectory() as temp_dir:
            control_path = Path(temp_dir) / "operator_controls.json"
            control_path.write_text(
                json.dumps(
                    {
                        "paused_robots": ["all"],
                        "priority_zones": [{"room": "n_medium"}],
                        "no_go_zones": [{"room": "s_medium"}],
                        "redirects": [{"robot": "epuck_1", "room": "ne_large"}],
                        "sim_reset_request": {"id": "old-reset"},
                    }
                ),
                encoding="utf-8",
            )

            changed = supervisor.reset_operator_control_file(control_path)
            reloaded = json.loads(control_path.read_text(encoding="utf-8"))

        self.assertTrue(changed)
        self.assertEqual(supervisor.default_operator_control_config(), reloaded)

    def test_startup_operator_control_reset_preserves_preloaded_zones(self):
        supervisor = load_supervisor_module()

        with tempfile.TemporaryDirectory() as temp_dir:
            control_path = Path(temp_dir) / "operator_controls.json"
            control_path.write_text(
                json.dumps(
                    {
                        "paused_robots": ["epuck_2"],
                        "priority_zones": [{"room": "ne_large", "weight": 2.0}],
                        "no_go_zones": [{"room": "se_small"}],
                        "redirects": [{"robot": "epuck_1", "room": "n_medium"}],
                        "sim_reset_request": {"id": "old-reset"},
                    }
                ),
                encoding="utf-8",
            )

            changed = supervisor.reset_operator_control_file(
                control_path,
                preserve_zones=True,
            )
            reloaded = json.loads(control_path.read_text(encoding="utf-8"))

        self.assertTrue(changed)
        self.assertEqual([], reloaded["paused_robots"])
        self.assertEqual([], reloaded["redirects"])
        self.assertIsNone(reloaded["sim_reset_request"])
        self.assertEqual("ne_large", reloaded["priority_zones"][0]["room"])
        self.assertEqual("se_small", reloaded["no_go_zones"][0]["room"])

    def test_reset_operator_state_file_clears_stale_dashboard_map(self):
        supervisor = load_supervisor_module()

        with tempfile.TemporaryDirectory() as temp_dir:
            state_path = Path(temp_dir) / "operator_state.json"
            state_path.write_text(
                json.dumps(
                    {
                        "step": 900,
                        "rooms": [{"name": "n_medium", "progress_percent": 70.0}],
                        "robots": [{"name": "epuck_1"}],
                    }
                ),
                encoding="utf-8",
            )

            changed = supervisor.reset_operator_state_file(state_path)
            reloaded = json.loads(state_path.read_text(encoding="utf-8"))

        self.assertTrue(changed)
        self.assertIsNone(reloaded["step"])
        self.assertEqual([], reloaded["robots"])
        self.assertEqual(0.0, reloaded["rooms"][0]["progress_percent"])

    def test_initial_assignment_uses_operator_priority_zones(self):
        supervisor = load_supervisor_module()
        allocator = supervisor.TaskAllocator(
            supervisor.ROOM_TASKS,
            supervisor.MRTA_DISTANCE_WEIGHT,
            supervisor.MRTA_AREA_WEIGHT,
        )
        robot_statuses = {
            robot_name: {"pose": {"x_m": 0.0, "y_m": 0.0}}
            for robot_name in supervisor.EXPECTED_ROBOTS
        }
        priority_zone = supervisor.normalize_operator_zone(
            {"name": "south small priority", "room": "se_small"},
            "priority",
        )

        base_assignments = allocator.assign(robot_statuses)
        priority_assignments = allocator.assign(
            robot_statuses,
            priority_zones=[priority_zone],
        )

        base_rooms = {assignment["room"] for assignment in base_assignments.values()}
        priority_rooms = {
            assignment["room"]
            for assignment in priority_assignments.values()
        }
        self.assertNotIn("se_small", base_rooms)
        self.assertIn("se_small", priority_rooms)

    def test_initial_assignment_with_large_priority_weight_finds_true_minimum(self):
        supervisor = load_supervisor_module()
        allocator = supervisor.TaskAllocator(
            supervisor.ROOM_TASKS,
            supervisor.MRTA_DISTANCE_WEIGHT,
            supervisor.MRTA_AREA_WEIGHT,
        )
        robot_statuses = {
            "epuck_1": {
                "pose": {"x_m": 0.6159725340577853, "y_m": -1.8978273792131863}
            },
            "epuck_2": {
                "pose": {"x_m": -1.3378081145917828, "y_m": -0.8240964513100117}
            },
            "epuck_3": {
                "pose": {"x_m": -0.7335174265472748, "y_m": -2.0366519538841477}
            },
            "epuck_4": {
                "pose": {"x_m": 1.8842594030169204, "y_m": 2.6627546972054557}
            },
        }
        priority_zones = [
            supervisor.normalize_operator_zone(
                {"name": "west priority", "room": "sw_large", "weight": 10.0},
                "priority",
            )
        ]

        assignments = allocator.assign(robot_statuses, priority_zones=priority_zones)
        assigned_rooms = {
            robot: assignment["room"] for robot, assignment in assignments.items()
        }
        assigned_total = sum(
            allocator.assignment_cost(
                robot_statuses[robot],
                room,
                priority_zones=priority_zones,
            )
            for robot, room in assigned_rooms.items()
        )

        robots = list(supervisor.EXPECTED_ROBOTS)
        best_total = None
        best_rooms = None
        for rooms in itertools.permutations(supervisor.ROOM_TASKS, len(robots)):
            total = sum(
                allocator.assignment_cost(
                    robot_statuses[robot],
                    room,
                    priority_zones=priority_zones,
                )
                for robot, room in zip(robots, rooms)
            )
            if best_total is None or total < best_total:
                best_total = total
                best_rooms = dict(zip(robots, rooms))

        self.assertEqual(best_rooms, assigned_rooms)
        self.assertAlmostEqual(best_total, assigned_total)
        self.assertEqual("nw_small", assigned_rooms["epuck_2"])

    def test_assignment_route_inserts_detour_around_operator_no_go_zone(self):
        supervisor = load_supervisor_module()
        no_go_zone = {
            "name": "hub block",
            "bounds": [-0.2, 0.2, -0.2, 0.2],
        }
        start_point = [-1.0, 0.0]
        route = [[1.0, 0.0]]

        adjusted_route = supervisor.route_around_no_go_zones(
            route,
            [no_go_zone],
            start_point=start_point,
        )

        self.assertGreater(len(adjusted_route), len(route))
        points = [start_point] + adjusted_route
        for index in range(len(points) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    points[index],
                    points[index + 1],
                    no_go_zone["bounds"],
                )
            )

    def test_assignment_route_can_leave_operator_no_go_covering_start_pose(self):
        supervisor = load_supervisor_module()
        no_go_zone = {
            "name": "hub block",
            "bounds": [-0.35, 0.35, -0.35, 0.35],
        }

        adjusted_route = supervisor.route_around_no_go_zones(
            [[1.0, 0.0]],
            [no_go_zone],
            start_point=[0.0, 0.0],
        )

        self.assertTrue(
            supervisor.route_reaches_final_waypoint([[1.0, 0.0]], adjusted_route)
        )
        self.assertFalse(
            supervisor.point_blocked_by_no_go_zones(adjusted_route[-1], [no_go_zone])
        )

        allocator = supervisor.TaskAllocator(
            supervisor.ROOM_TASKS,
            supervisor.MRTA_DISTANCE_WEIGHT,
            supervisor.MRTA_AREA_WEIGHT,
        )
        robot_statuses = {
            "epuck_1": {"pose": {"x_m": -0.35, "y_m": 0.30}},
            "epuck_2": {"pose": {"x_m": -0.12, "y_m": 0.15}},
            "epuck_3": {"pose": {"x_m": 0.12, "y_m": -0.15}},
            "epuck_4": {"pose": {"x_m": 0.35, "y_m": -0.30}},
        }

        assignments = allocator.assign(robot_statuses, no_go_zones=[no_go_zone])

        self.assertEqual(set(supervisor.EXPECTED_ROBOTS), set(assignments))
        self.assertTrue(
            all(not math.isinf(assignment["cost"]) for assignment in assignments.values())
        )

    def test_assignment_route_skips_operator_blocked_doorway_waypoints(self):
        supervisor = load_supervisor_module()
        status = {"pose": {"x_m": 0.0, "y_m": 0.0}}
        assignment = supervisor.build_assignment(status, "n_medium")
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        coverage_plan = supervisor.prepare_assignment_for_dispatch(
            "epuck_1",
            assignment,
            robot_status=status,
            room_assignments={"epuck_1": assignment},
            no_go_zones=[no_go_zone],
        )

        self.assertEqual([], coverage_plan)
        self.assertEqual([0.0, 0.0], assignment["target"])
        self.assertEqual([[0.0, 0.0]], assignment["route"])
        self.assertNotIn([-0.4, 0.57], assignment["route"])
        self.assertNotIn([-0.4, 0.93], assignment["route"])
        for waypoint in assignment["route"]:
            self.assertFalse(
                supervisor.point_blocked_by_no_go_zones(waypoint, [no_go_zone])
            )

        points = [[status["pose"]["x_m"], status["pose"]["y_m"]]] + assignment["route"]
        for index in range(len(points) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    points[index],
                    points[index + 1],
                    no_go_zone["bounds"],
                )
            )

    def test_allocator_skips_rooms_with_operator_blocked_doorways(self):
        supervisor = load_supervisor_module()
        allocator = supervisor.TaskAllocator(
            supervisor.ROOM_TASKS,
            supervisor.MRTA_DISTANCE_WEIGHT,
            supervisor.MRTA_AREA_WEIGHT,
        )
        robot_statuses = {
            robot: {"pose": {"x_m": 0.0, "y_m": 0.0}}
            for robot in supervisor.EXPECTED_ROBOTS
        }
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }

        assignments = allocator.assign(robot_statuses, no_go_zones=[no_go_zone])
        assigned_rooms = {assignment["room"] for assignment in assignments.values()}

        self.assertTrue(
            math.isinf(
                allocator.assignment_cost(
                    robot_statuses["epuck_1"],
                    "n_medium",
                    no_go_zones=[no_go_zone],
                )
            )
        )
        self.assertTrue(
            math.isinf(
                supervisor.route_cost_to_room(
                    robot_statuses["epuck_1"],
                    "n_medium",
                    [no_go_zone],
                )
            )
        )
        self.assertEqual({"sw_large", "s_medium", "se_small"}, assigned_rooms)
        self.assertEqual(3, len(assignments))

    def test_no_go_detour_rechecks_inserted_segments_against_all_zones(self):
        supervisor = load_supervisor_module()
        no_go_zones = [
            {"name": "upper block", "bounds": [-0.58, -0.22, -0.18, 0.18]},
            {"name": "lower block", "bounds": [-0.58, -0.22, -0.58, -0.22]},
        ]
        start_point = [-1.0, 0.0]
        route = [[1.0, 0.0]]

        adjusted_route = supervisor.route_around_no_go_zones(
            route,
            no_go_zones,
            start_point=start_point,
        )

        points = [start_point] + adjusted_route
        for index in range(len(points) - 1):
            for zone in no_go_zones:
                self.assertFalse(
                    supervisor.segment_intersects_bounds(
                        points[index],
                        points[index + 1],
                        zone["bounds"],
                    )
                )

    def test_no_go_route_reports_unreachable_when_no_clear_detour_exists(self):
        supervisor = load_supervisor_module()
        no_go_zones = [
            {"name": "top block", "bounds": [-0.2, 0.2, 0.2, 0.3]},
            {"name": "bottom block", "bounds": [-0.2, 0.2, -0.3, -0.2]},
            {"name": "left block", "bounds": [-0.3, -0.2, -0.3, 0.3]},
            {"name": "right block", "bounds": [0.2, 0.3, -0.3, 0.3]},
        ]
        start_point = [0.0, 0.0]
        route = [[1.0, 0.0]]

        adjusted_route = supervisor.route_around_no_go_zones(
            route,
            no_go_zones,
            start_point=start_point,
        )

        self.assertFalse(
            supervisor.route_reaches_final_waypoint(route, adjusted_route)
        )
        points = [start_point] + adjusted_route
        for index in range(len(points) - 1):
            for zone in no_go_zones:
                self.assertFalse(
                    supervisor.segment_intersects_bounds(
                        points[index],
                        points[index + 1],
                        zone["bounds"],
                    )
                )

    def test_route_cost_includes_operator_no_go_detour(self):
        supervisor = load_supervisor_module()
        status = {"pose": {"x_m": -1.0, "y_m": 0.0}}
        no_go_zone = {
            "name": "hub block",
            "bounds": [-0.2, 0.2, -0.2, 0.2],
        }

        base_cost = supervisor.route_cost_to_room(status, "se_small")
        detour_cost = supervisor.route_cost_to_room(status, "se_small", [no_go_zone])

        self.assertGreater(detour_cost, base_cost)

    def test_coverage_plan_skips_and_detours_operator_no_go_zone(self):
        supervisor = load_supervisor_module()
        no_go_zone = {
            "name": "blocked tile",
            "bounds": [-0.1, 0.1, -0.1, 0.1],
        }
        coverage_plan = [[-0.5, 0.0], [0.0, 0.0], [0.5, 0.0]]

        adjusted_plan = supervisor.coverage_plan_around_no_go_zones(
            coverage_plan,
            [no_go_zone],
        )

        self.assertNotIn([0.0, 0.0], adjusted_plan)
        self.assertGreater(len(adjusted_plan), 2)
        for waypoint in adjusted_plan:
            self.assertFalse(
                supervisor.point_blocked_by_no_go_zones(waypoint, [no_go_zone])
            )
        for index in range(len(adjusted_plan) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    adjusted_plan[index],
                    adjusted_plan[index + 1],
                    no_go_zone["bounds"],
                )
            )

    def test_assignment_route_exits_current_no_go_room_on_resume(self):
        supervisor = load_supervisor_module()
        status = {"pose": {"x_m": -1.573, "y_m": 1.154}}
        assignment = supervisor.build_assignment(status, "n_medium")
        no_go_zone = supervisor.normalize_operator_zone(
            {"name": "no-go nw_small", "room": "nw_small"},
            "no_go",
        )

        coverage_plan = supervisor.prepare_assignment_for_dispatch(
            "epuck_1",
            assignment,
            robot_status=status,
            room_assignments={"epuck_1": assignment},
            no_go_zones=[no_go_zone],
        )

        self.assertGreater(len(coverage_plan), 0)
        self.assertEqual(coverage_plan[0], assignment["target"])
        nw_inside, nw_hub = supervisor.room_doorway_waypoints("nw_small")
        n_inside, n_hub = supervisor.room_doorway_waypoints("n_medium")
        self.assertIn(nw_inside, assignment["route"])
        self.assertIn(nw_hub, assignment["route"])
        self.assertIn(n_hub, assignment["route"])
        self.assertIn(n_inside, assignment["route"])
        self.assertEqual(coverage_plan[0], assignment["route"][-1])
        self.assertFalse(
            supervisor.point_blocked_by_no_go_zones(
                assignment["target"],
                [no_go_zone],
            )
        )
        divider_wall_bounds = [-1.51, -1.49, 0.75, 3.0]
        route_points = [
            [status["pose"]["x_m"], status["pose"]["y_m"]],
        ] + assignment["route"]
        for index in range(len(route_points) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    route_points[index],
                    route_points[index + 1],
                    divider_wall_bounds,
                )
            )

        exited_no_go = False
        for waypoint in assignment["route"]:
            blocked = supervisor.point_blocked_by_no_go_zones(
                waypoint,
                [no_go_zone],
            )
            if not blocked:
                exited_no_go = True
            elif exited_no_go:
                self.fail("route re-entered no-go room after exiting")

    def test_room_sized_no_go_assignment_stops_outside_blocked_room(self):
        supervisor = load_supervisor_module()
        status = {"pose": {"x_m": 0.0, "y_m": 0.0}}
        assignment = supervisor.build_assignment(status, "n_medium")
        no_go_zone = supervisor.normalize_operator_zone(
            {"name": "blocked north", "room": "n_medium"},
            "no_go",
        )

        coverage_plan = supervisor.prepare_assignment_for_dispatch(
            "epuck_1",
            assignment,
            robot_status=status,
            room_assignments={"epuck_1": assignment},
            no_go_zones=[no_go_zone],
        )

        self.assertEqual([], coverage_plan)
        self.assertFalse(
            supervisor.point_blocked_by_no_go_zones(
                assignment["target"],
                [no_go_zone],
            )
        )
        for waypoint in assignment["route"]:
            self.assertFalse(
                supervisor.point_blocked_by_no_go_zones(waypoint, [no_go_zone])
            )

    def test_room_coverage_plan_detours_first_leg_from_robot_pose(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        no_go_zone = {
            "name": "new block",
            "bounds": [-1.35, -1.25, 1.05, 1.25],
        }
        robot_pose = {"x_m": -1.3, "y_m": 1.4}
        room_assignments = {"epuck_1": {"room": "n_medium"}}

        plans = supervisor.send_room_coverage_plans(
            FakeEmitter(),
            room_assignments,
            "n_medium",
            no_go_zones=[no_go_zone],
            latest_robot_status={"epuck_1": {"pose": robot_pose}},
        )

        plan = plans["epuck_1"]
        self.assertGreater(len(plan), 0)
        points = [[robot_pose["x_m"], robot_pose["y_m"]]] + plan
        for index in range(len(points) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    points[index],
                    points[index + 1],
                    no_go_zone["bounds"],
                )
            )

    def test_room_coverage_plan_keeps_sweep_start_for_en_route_robot(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        no_go_zone = {
            "name": "hub leg",
            "bounds": [0.0, 0.5, -0.45, -0.05],
        }
        room_assignments = {"epuck_1": {"room": "se_small"}}
        status = {
            "pose": {"x_m": -1.0, "y_m": 0.0},
            "assignment_target_reached": False,
        }
        expected_plan = supervisor.coverage_plan_around_no_go_zones(
            supervisor.generate_coverage_waypoints("se_small"),
            [no_go_zone],
        )

        plans = supervisor.send_room_coverage_plans(
            FakeEmitter(),
            room_assignments,
            "se_small",
            no_go_zones=[no_go_zone],
            latest_robot_status={"epuck_1": status},
        )

        self.assertEqual(expected_plan, plans["epuck_1"])
        self.assertEqual([1.7, -0.95], plans["epuck_1"][0])

    def test_room_coverage_plan_ignores_stale_target_reached_from_other_room(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        no_go_zone = {
            "name": "old room exit block",
            "bounds": [-0.8, 0.2, 0.8, 1.4],
        }
        room_assignments = {"epuck_1": {"room": "se_small"}}
        status = {
            "pose": {"x_m": -0.4, "y_m": 1.875},
            "assignment_target_reached": True,
        }
        expected_plan = supervisor.coverage_plan_around_no_go_zones(
            supervisor.generate_coverage_waypoints("se_small"),
            [no_go_zone],
        )

        plans = supervisor.send_room_coverage_plans(
            FakeEmitter(),
            room_assignments,
            "se_small",
            no_go_zones=[no_go_zone],
            latest_robot_status={"epuck_1": status},
        )

        self.assertEqual(expected_plan, plans["epuck_1"])
        self.assertEqual([1.7, -0.95], plans["epuck_1"][0])

    def test_room_coverage_plan_suppressed_when_room_entry_blocked(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        no_go_zone = {
            "name": "North doorways",
            "bounds": [-2.55, 2.15, 0.45, 1.05],
        }
        room_assignments = {"epuck_1": {"room": "n_medium"}}

        plans = supervisor.send_room_coverage_plans(
            emitter,
            room_assignments,
            "n_medium",
            no_go_zones=[no_go_zone],
            latest_robot_status={
                "epuck_1": {"pose": {"x_m": 0.0, "y_m": 0.0}},
            },
        )

        self.assertEqual([], plans["epuck_1"])
        self.assertEqual([], emitter.messages[0]["waypoints"])

    def test_traffic_reservations_stagger_shared_hub_routes(self):
        supervisor = load_supervisor_module()
        reservations = supervisor.TrafficReservationBook()
        resources = ["hub:all", "hub:north", "doorway:n_medium"]

        first = reservations.reserve("epuck_1", resources, step_count=10)
        second = reservations.reserve("epuck_2", resources, step_count=10)

        self.assertEqual(0, first["start_delay_steps"])
        self.assertEqual(
            supervisor.TRAFFIC_RESOURCE_STAGGER_STEPS,
            second["start_delay_steps"],
        )
        self.assertGreater(second["conflict_count"], 0)
        self.assertEqual(1, second["doorway_conflicts"])

    def test_send_assignment_commands_targets_one_robot(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        assignment = {
            "room": "n_medium",
            "target": supervisor.ROOM_TASKS["n_medium"]["center"],
            "cost": 0.5,
            "route": [[0.0, 0.0], [-0.4, 0.57], [-0.4, 0.93], [-0.4, 1.875]],
        }

        coverage_plan = supervisor.send_assignment_commands(
            emitter,
            "epuck_2",
            assignment,
        )

        self.assertEqual(
            supervisor.generate_coverage_waypoints("n_medium"),
            coverage_plan,
        )
        self.assertEqual(2, len(emitter.messages))

        task_command = json.loads(emitter.messages[0])
        plan_command = json.loads(emitter.messages[1])
        self.assertEqual("task_assignment", task_command["type"])
        self.assertEqual("coverage_plan", plan_command["type"])
        self.assertEqual("epuck_2", task_command["robot"])
        self.assertEqual("epuck_2", plan_command["robot"])
        self.assertEqual("n_medium", task_command["room"])
        self.assertEqual("n_medium", plan_command["room"])
        self.assertEqual(assignment["route"], task_command["route"])
        self.assertEqual("sweep", plan_command["plan_kind"])
        self.assertEqual(0, plan_command["lane_index"])
        self.assertEqual(1, plan_command["lane_count"])

    def test_send_assignment_commands_adds_route_delay_and_metrics(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        reservations = supervisor.TrafficReservationBook()
        status = {"pose": {"x_m": -0.3, "y_m": 0.1}}
        first_assignment = supervisor.build_assignment(status, "n_medium")
        second_assignment = supervisor.build_assignment(status, "n_medium")

        supervisor.send_assignment_commands(
            FakeEmitter(),
            "epuck_1",
            first_assignment,
            {"epuck_1": first_assignment},
            robot_status=status,
            traffic_reservations=reservations,
            step_count=20,
        )

        emitter = FakeEmitter()
        supervisor.send_assignment_commands(
            emitter,
            "epuck_2",
            second_assignment,
            {"epuck_2": second_assignment},
            robot_status=status,
            traffic_reservations=reservations,
            step_count=20,
        )

        command = json.loads(emitter.messages[0])
        self.assertEqual(
            supervisor.TRAFFIC_RESOURCE_STAGGER_STEPS,
            command["start_delay_steps"],
        )
        self.assertGreater(command["path_metrics"]["planned_distance_m"], 0.0)
        self.assertIn("hub:all", command["path_metrics"]["resources"])
        self.assertGreater(command["path_metrics"]["doorway_conflicts"], 0)

    def test_send_assignment_commands_routes_around_operator_no_go_zone(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        status = {"pose": {"x_m": -1.0, "y_m": 0.0}}
        assignment = supervisor.build_assignment(status, "se_small")
        original_route_len = len(assignment["route"])
        no_go_zone = {
            "name": "hub block",
            "bounds": [-0.2, 0.2, -0.2, 0.2],
        }

        supervisor.send_assignment_commands(
            emitter,
            "epuck_1",
            assignment,
            {"epuck_1": assignment},
            robot_status=status,
            no_go_zones=[no_go_zone],
        )

        command = json.loads(emitter.messages[0])
        self.assertIn("hub block", command["path_metrics"]["operator_no_go_zones"])
        self.assertGreater(len(command["route"]), original_route_len)
        points = [[-1.0, 0.0]] + command["route"]
        for index in range(len(points) - 1):
            self.assertFalse(
                supervisor.segment_intersects_bounds(
                    points[index],
                    points[index + 1],
                    no_go_zone["bounds"],
                )
            )

    def test_send_assignment_commands_uses_split_lane_when_room_has_helpers(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large"},
        }
        assignment = {
            "room": "ne_large",
            "target": supervisor.ROOM_TASKS["ne_large"]["center"],
            "cost": 0.5,
            "route": [[0.0, 0.0], [1.85, 0.57], [1.85, 0.93], [1.85, 1.875]],
        }

        coverage_plan = supervisor.send_assignment_commands(
            emitter,
            "epuck_2",
            assignment,
            room_assignments,
        )

        plan_command = json.loads(emitter.messages[1])
        self.assertEqual(
            supervisor.generate_coverage_waypoints("ne_large", 1, 2),
            coverage_plan,
        )
        self.assertEqual(1, plan_command["lane_index"])
        self.assertEqual(2, plan_command["lane_count"])
        self.assertEqual(coverage_plan, plan_command["waypoints"])

    def test_send_room_coverage_plans_resplits_all_robots_in_room(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_3": {"room": "s_medium"},
        }

        plans = supervisor.send_room_coverage_plans(
            emitter,
            room_assignments,
            "ne_large",
        )

        self.assertEqual({"epuck_1", "epuck_2"}, set(plans))
        self.assertEqual(2, len(emitter.messages))
        first_plan = json.loads(emitter.messages[0])
        second_plan = json.loads(emitter.messages[1])
        self.assertEqual("epuck_1", first_plan["robot"])
        self.assertEqual(0, first_plan["lane_index"])
        self.assertEqual(2, first_plan["lane_count"])
        self.assertEqual("epuck_2", second_plan["robot"])
        self.assertEqual(1, second_plan["lane_index"])
        self.assertEqual(2, second_plan["lane_count"])

    def test_no_go_route_refresh_retargets_robot_waiting_outside_room(self):
        supervisor = load_supervisor_module()
        assignment = {"room": "se_small"}

        self.assertTrue(
            supervisor.assignment_route_needs_refresh(
                {"assignment_target_reached": True},
                assignment,
                {"x_m": 2.25, "y_m": -0.55},
            )
        )
        self.assertFalse(
            supervisor.assignment_route_needs_refresh(
                {"assignment_target_reached": True},
                assignment,
                {"x_m": 2.25, "y_m": -1.875},
            )
        )
        self.assertTrue(
            supervisor.assignment_route_needs_refresh(
                {"assignment_target_reached": False},
                assignment,
                {"x_m": 2.25, "y_m": -1.875},
            )
        )

    def test_room_work_plan_uses_cleanup_when_dirty_tiles_fit_helpers(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"sw_large": 80}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("sw_large", col_index, 0)
            for col_index in range(11)
        }
        overlay.tile_centers = {
            ("sw_large", col_index, 0): [
                round(-2.8 + 0.25 * col_index, 3),
                -0.875,
            ]
            for col_index in range(11)
        }
        room_assignments = {
            "epuck_1": {"room": "sw_large"},
            "epuck_2": {"room": "sw_large", "helper": True},
            "epuck_3": {"room": "sw_large", "helper": True},
        }
        latest_robot_status = {
            "epuck_1": {"pose": {"x_m": -2.6, "y_m": -1.0}},
            "epuck_2": {"pose": {"x_m": -2.0, "y_m": -1.0}},
            "epuck_3": {"pose": {"x_m": -1.4, "y_m": -1.0}},
        }
        cleanup_plan_signatures = {}
        cleanup_plan_steps = {}

        plans = supervisor.send_room_work_plans(
            emitter,
            room_assignments,
            "sw_large",
            overlay,
            cleanup_plan_signatures,
            cleanup_plan_steps,
            latest_robot_status=latest_robot_status,
            step_count=50,
        )

        self.assertEqual({"epuck_1", "epuck_2", "epuck_3"}, set(plans))
        self.assertEqual(
            ["cleanup", "cleanup", "cleanup"],
            [message["plan_kind"] for message in emitter.messages],
        )
        self.assertEqual(11, len(overlay.tile_claims))
        self.assertLess(len(plans["epuck_3"]), supervisor.CLEANUP_TARGETS_PER_ROBOT)

    def test_final_room_cleanup_keeps_primary_robot_moving(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        overlay.room_tile_counts = {"sw_large": 80}
        overlay.tile_claims = {}
        overlay.dirty_tiles = {
            ("sw_large", col_index, 0)
            for col_index in range(3)
        }
        overlay.tile_centers = {
            ("sw_large", col_index, 0): [
                round(-2.8 + 0.25 * col_index, 3),
                -0.875,
            ]
            for col_index in range(3)
        }
        room_assignments = {
            "epuck_3": {"room": "sw_large"},
            "epuck_4": {"room": "sw_large", "helper": True},
        }
        latest_robot_status = {
            "epuck_3": {"pose": {"x_m": -1.0, "y_m": -1.0}},
            "epuck_4": {"pose": {"x_m": -2.6, "y_m": -1.0}},
        }

        plans = supervisor.send_room_cleanup_plans(
            emitter,
            room_assignments,
            "sw_large",
            overlay,
            {},
            {},
            800,
            "final dirty tiles remain",
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual([message["robot"] for message in emitter.messages], ["epuck_3", "epuck_4"])
        self.assertGreater(len(plans["epuck_3"]), 0)
        self.assertEqual([], plans["epuck_4"])
        self.assertEqual({"epuck_3"}, set(overlay.tile_claims.values()))

    def test_completed_cleanup_plan_refreshes_when_dirty_tile_remains(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        tile_key = ("se_small", 0, 0)
        tile_center = [1.625, -1.875]
        overlay.room_tile_counts = {"se_small": 54}
        overlay.dirty_tiles = {tile_key}
        overlay.tile_centers = {tile_key: tile_center}
        overlay.tile_claims = {tile_key: "epuck_4"}
        cleanup_plan_signatures = {
            "epuck_4": ("se_small", (tuple(tile_center),)),
        }
        cleanup_plan_steps = {"epuck_4": 200}
        coverage = {
            "room": "se_small",
            "complete": True,
            "plan_kind": "cleanup",
        }

        reset = supervisor.reset_completed_cleanup_plan_if_dirty(
            "epuck_4",
            "se_small",
            coverage,
            overlay,
            cleanup_plan_signatures,
            cleanup_plan_steps,
        )
        cleanup_plan = supervisor.send_cleanup_plan_if_needed(
            emitter,
            overlay,
            "epuck_4",
            "se_small",
            {"x_m": 1.7, "y_m": -1.9},
            cleanup_plan_signatures,
            cleanup_plan_steps,
            210,
            "sweep complete; 1 dirty tile remains",
        )

        self.assertTrue(reset)
        self.assertEqual(
            [supervisor.cleanup_waypoint_for_tile_center("se_small", tile_center)],
            cleanup_plan,
        )
        self.assertEqual("epuck_4", overlay.tile_claims[tile_key])
        self.assertEqual("coverage_plan", emitter.messages[0]["type"])

    def test_room_cleanup_retry_releases_stale_claims_after_all_done(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        tile_key = ("sw_large", 0, 0)
        tile_center = [-2.875, -0.875]
        overlay.room_tile_counts = {"sw_large": 80}
        overlay.dirty_tiles = {tile_key}
        overlay.tile_centers = {tile_key: tile_center}
        overlay.tile_claims = {tile_key: "epuck_3"}
        room_assignments = {
            "epuck_3": {"room": "sw_large"},
            "epuck_4": {"room": "sw_large", "helper": True},
        }
        latest_robot_status = {
            robot_name: {
                "pose": {"x_m": -2.4, "y_m": -1.0},
                "coverage": {
                    "room": "sw_large",
                    "complete": True,
                    "plan_kind": "cleanup",
                },
            }
            for robot_name in room_assignments
        }
        cleanup_plan_signatures = {
            "epuck_3": ("sw_large", (tuple(tile_center),)),
            "epuck_4": ("sw_large", ()),
        }
        cleanup_plan_steps = {"epuck_3": 200, "epuck_4": 200}

        plans = supervisor.send_room_cleanup_retry_if_all_done(
            emitter,
            room_assignments,
            "sw_large",
            overlay,
            cleanup_plan_signatures,
            cleanup_plan_steps,
            230,
            "dirty tile remains after completed cleanup",
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual({"epuck_3", "epuck_4"}, set(plans))
        self.assertEqual(
            [supervisor.cleanup_waypoint_for_tile_center("sw_large", tile_center)],
            plans["epuck_3"],
        )
        self.assertEqual([], plans["epuck_4"])
        self.assertEqual("epuck_3", overlay.tile_claims[tile_key])
        self.assertEqual("coverage_plan", emitter.messages[0]["type"])
        self.assertEqual("cleanup", emitter.messages[0]["plan_kind"])

    def test_retry_completed_cleanup_rooms_finds_stopped_dirty_room(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(json.loads(payload))

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.rooms = supervisor.ROOM_TASKS
        tile_key = ("n_medium", 0, 0)
        tile_center = [-1.375, 0.875]
        overlay.room_tile_counts = {room: 1 for room in supervisor.ROOM_TASKS}
        overlay.dirty_tiles = {tile_key}
        overlay.tile_centers = {tile_key: tile_center}
        overlay.tile_claims = {tile_key: "epuck_3"}
        room_assignments = {
            "epuck_1": {"room": "n_medium"},
            "epuck_3": {"room": "n_medium", "helper": True},
        }
        latest_robot_status = {
            robot_name: {
                "pose": {"x_m": -1.0, "y_m": 1.0},
                "coverage": {
                    "room": "n_medium",
                    "complete": True,
                    "plan_kind": "cleanup",
                },
            }
            for robot_name in room_assignments
        }

        retried_rooms, plans = supervisor.retry_completed_cleanup_rooms(
            emitter,
            room_assignments,
            set(supervisor.ROOM_TASKS) - {"n_medium"},
            overlay,
            {
                "epuck_1": ("n_medium", ()),
                "epuck_3": ("n_medium", (tuple(tile_center),)),
            },
            {"epuck_1": 500, "epuck_3": 500},
            550,
            latest_robot_status=latest_robot_status,
        )

        self.assertEqual({"n_medium"}, retried_rooms)
        self.assertEqual({"epuck_1", "epuck_3"}, set(plans))
        self.assertEqual(
            [supervisor.cleanup_waypoint_for_tile_center("n_medium", tile_center)],
            plans["epuck_1"],
        )
        self.assertEqual("epuck_1", overlay.tile_claims[tile_key])

    def test_resplitting_room_clears_overwritten_cleanup_state(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        overlay = supervisor.CleaningOverlay.__new__(supervisor.CleaningOverlay)
        overlay.tile_claims = {
            ("ne_large", 0, 0): "epuck_1",
            ("ne_large", 1, 0): "epuck_2",
            ("s_medium", 0, 0): "epuck_3",
        }
        cleanup_plan_signatures = {
            "epuck_1": ("ne_large", ((0.875, 0.875),)),
            "epuck_2": ("ne_large", ((1.125, 0.875),)),
            "epuck_3": ("s_medium", ((-0.625, -2.875),)),
        }
        cleanup_plan_steps = {
            "epuck_1": 10,
            "epuck_2": 20,
            "epuck_3": 30,
        }
        room_assignments = {
            "epuck_1": {"room": "ne_large"},
            "epuck_2": {"room": "ne_large", "helper": True},
            "epuck_3": {"room": "s_medium"},
        }

        supervisor.send_room_coverage_plans(
            emitter,
            room_assignments,
            "ne_large",
            overlay,
            cleanup_plan_signatures,
            cleanup_plan_steps,
        )

        self.assertEqual({"epuck_3"}, set(cleanup_plan_signatures))
        self.assertEqual({"epuck_3"}, set(cleanup_plan_steps))
        self.assertEqual(
            {("s_medium", 0, 0): "epuck_3"},
            overlay.tile_claims,
        )

    def test_send_recovery_command_targets_one_robot(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        supervisor.send_recovery_command(emitter, "epuck_3", "blocked")

        self.assertEqual(1, len(emitter.messages))
        command = json.loads(emitter.messages[0])
        self.assertEqual("recovery", command["type"])
        self.assertEqual("epuck_3", command["robot"])
        self.assertEqual("blocked", command["reason"])
        self.assertEqual(
            supervisor.ROBOT_RECOVERY_REVERSE_STEPS,
            command["reverse_steps"],
        )
        self.assertEqual(supervisor.ROBOT_RECOVERY_TURN_STEPS, command["turn_steps"])

    def test_send_idle_command_targets_one_robot(self):
        supervisor = load_supervisor_module()

        class FakeEmitter:
            def __init__(self):
                self.messages = []

            def send(self, payload):
                self.messages.append(payload)

        emitter = FakeEmitter()
        supervisor.send_idle_command(emitter, "epuck_4")

        self.assertEqual(1, len(emitter.messages))
        command = json.loads(emitter.messages[0])
        self.assertEqual({"type": "idle", "robot": "epuck_4"}, command)

    def test_robot_grid_uses_repeated_free_evidence(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        self.assertFalse(grid.mark_free_cell(5, 5))
        self.assertEqual(controller.GRID_UNKNOWN, grid.data[5][5])

        self.assertTrue(grid.mark_free_cell(5, 5))
        self.assertEqual(controller.GRID_FREE, grid.data[5][5])
        self.assertEqual(1, grid.free_cell_count)

    def test_robot_grid_wall_evidence_can_override_light_free_evidence(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        grid.mark_free_cell(5, 5)
        grid.mark_free_cell(5, 5)
        self.assertEqual(controller.GRID_FREE, grid.data[5][5])

        grid.mark_wall_cell(5, 5)
        self.assertEqual(controller.GRID_UNKNOWN, grid.data[5][5])
        self.assertEqual(0, grid.free_cell_count)
        self.assertEqual(0, grid.wall_cell_count)

        grid.mark_wall_cell(5, 5)
        self.assertEqual(controller.GRID_WALL, grid.data[5][5])
        self.assertEqual(1, grid.wall_cell_count)

    def test_robot_grid_bad_wall_reading_can_be_recovered_by_free_evidence(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        grid.mark_wall_cell(5, 5)
        self.assertEqual(controller.GRID_WALL, grid.data[5][5])

        for _ in range(5):
            grid.mark_free_cell(5, 5)

        self.assertEqual(controller.GRID_FREE, grid.data[5][5])
        self.assertEqual(1, grid.free_cell_count)
        self.assertEqual(0, grid.wall_cell_count)

    def test_robot_grid_sends_observation_counts_with_legacy_cells(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        grid.mark_free_cell(5, 5)
        grid.mark_free_cell(5, 5)
        grid.mark_wall_cell(6, 5)
        updates = grid.drain_pending_updates()

        self.assertEqual([[5, 5, 2]], updates["free_observations"])
        self.assertEqual([[6, 5, 1]], updates["wall_observations"])
        self.assertEqual([[5, 5]], updates["free_cells"])
        self.assertEqual([[6, 5]], updates["wall_cells"])
        self.assertEqual(
            {
                "free_cells": [],
                "wall_cells": [],
                "free_observations": [],
                "wall_observations": [],
                "observations": [],
            },
            grid.drain_pending_updates(),
        )

    def test_robot_grid_sends_observations_in_order(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        for _ in range(3):
            grid.mark_wall_cell(5, 5)
        for _ in range(10):
            grid.mark_free_cell(5, 5)
        updates = grid.drain_pending_updates()

        self.assertEqual(
            [
                ["wall", 5, 5, 3],
                ["free", 5, 5, 10],
            ],
            updates["observations"],
        )

    def test_robot_grid_compacts_repeated_observations_per_cell(self):
        controller = load_controller_module()
        grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        grid.mark_free_cell(5, 5)
        grid.mark_free_cell(6, 5)
        grid.mark_free_cell(5, 5)
        grid.mark_wall_cell(5, 5)
        grid.mark_free_cell(6, 5)
        updates = grid.drain_pending_updates()

        self.assertEqual(
            [
                ["free", 5, 5, 2],
                ["wall", 5, 5, 1],
                ["free", 6, 5, 2],
            ],
            updates["observations"],
        )

    def test_supervisor_grid_merges_confidence_observations(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        free_count, wall_count = grid.merge_update(
            {"free_observations": [[5, 5, 1]]}
        )
        self.assertEqual((0, 0), (free_count, wall_count))
        self.assertEqual(supervisor.GRID_UNKNOWN, grid.data[5][5])

        free_count, wall_count = grid.merge_update(
            {"free_observations": [[5, 5, 1]]}
        )
        self.assertEqual((1, 0), (free_count, wall_count))
        self.assertEqual(supervisor.GRID_FREE, grid.data[5][5])

        free_count, wall_count = grid.merge_update(
            {"wall_observations": [[5, 5, 2]]}
        )
        self.assertEqual((0, 1), (free_count, wall_count))
        self.assertEqual(supervisor.GRID_WALL, grid.data[5][5])

    def test_supervisor_ordered_observations_match_robot_clamped_score(self):
        controller = load_controller_module()
        supervisor = load_supervisor_module()
        robot_grid = controller.OccupancyGrid(world_size_m=1.0, cell_size_m=0.1)
        global_grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        for _ in range(3):
            robot_grid.mark_wall_cell(5, 5)
        for _ in range(10):
            robot_grid.mark_free_cell(5, 5)

        global_grid.merge_update(robot_grid.drain_pending_updates())

        self.assertEqual(robot_grid.scores[5][5], global_grid.scores[5][5])
        self.assertEqual(robot_grid.data[5][5], global_grid.data[5][5])
        self.assertEqual(supervisor.GRID_FREE, global_grid.data[5][5])

    def test_scan_match_shifts_update_toward_existing_wall_cells(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        for _ in range(2):
            grid.add_cell_evidence(5, 5, supervisor.WALL_EVIDENCE)
            grid.add_cell_evidence(5, 6, supervisor.WALL_EVIDENCE)
        map_update = {
            "observations": [
                ["wall", 6, 5, 2],
                ["wall", 6, 6, 2],
            ]
        }

        scan_match = grid.scan_match_update(map_update)

        self.assertTrue(scan_match["accepted"])
        self.assertEqual([-1, 0], scan_match["offset_cells"])
        self.assertEqual(
            [["wall", 5, 5, 2], ["wall", 5, 6, 2]],
            scan_match["map_update"]["observations"],
        )

    def test_scan_match_rejects_offsets_without_enough_existing_map_support(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        scan_match = grid.scan_match_update(
            {"observations": [["wall", 6, 5, 2]]}
        )

        self.assertFalse(scan_match["accepted"])
        self.assertEqual([0, 0], scan_match["offset_cells"])

    def test_scan_match_does_not_accept_free_space_only_offsets(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        for _ in range(2):
            grid.add_cell_evidence(5, 5, supervisor.FREE_EVIDENCE)
            grid.add_cell_evidence(5, 6, supervisor.FREE_EVIDENCE)

        scan_match = grid.scan_match_update(
            {
                "observations": [
                    ["free", 6, 5, 2],
                    ["free", 6, 6, 2],
                ]
            }
        )

        self.assertFalse(scan_match["accepted"])
        self.assertEqual([0, 0], scan_match["offset_cells"])

    def test_ground_truth_correction_is_only_fallback_for_stale_or_drifted_pose(self):
        supervisor = load_supervisor_module()
        actual_pose = {"x_m": 0.0, "y_m": 0.0, "theta_rad": 0.0}
        fresh_status = {
            "pose": {"x_m": 0.1, "y_m": 0.0, "theta_rad": 0.1},
            "localization": {"confidence": 0.9},
        }
        stale_status = {
            "pose": {"x_m": 0.1, "y_m": 0.0, "theta_rad": 0.1},
            "localization": {"confidence": 0.1},
        }
        drifted_status = {
            "pose": {"x_m": 1.0, "y_m": 0.0, "theta_rad": 0.1},
            "localization": {"confidence": 0.9},
        }

        self.assertFalse(
            supervisor.should_send_ground_truth_pose_correction(
                fresh_status,
                actual_pose,
            )
        )
        self.assertTrue(
            supervisor.should_send_ground_truth_pose_correction(
                stale_status,
                actual_pose,
            )
        )
        self.assertTrue(
            supervisor.should_send_ground_truth_pose_correction(
                drifted_status,
                actual_pose,
            )
        )

    def test_supervisor_grid_keeps_legacy_map_update_support(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        free_count, wall_count = grid.merge_update(
            {
                "free_cells": [[5, 5]],
                "wall_cells": [[6, 5]],
            }
        )

        self.assertEqual((1, 1), (free_count, wall_count))
        self.assertEqual(supervisor.GRID_FREE, grid.data[5][5])
        self.assertEqual(supervisor.GRID_WALL, grid.data[5][6])

    def test_supervisor_legacy_wall_update_still_overrides_free_cell(self):
        supervisor = load_supervisor_module()
        grid = supervisor.GlobalOccupancyGrid(world_size_m=1.0, cell_size_m=0.1)

        grid.merge_update({"free_cells": [[5, 5]]})
        free_count, wall_count = grid.merge_update({"wall_cells": [[5, 5]]})

        self.assertEqual((0, 1), (free_count, wall_count))
        self.assertEqual(supervisor.GRID_WALL, grid.data[5][5])
        self.assertEqual(0, grid.free_cell_count)
        self.assertEqual(1, grid.wall_cell_count)


if __name__ == "__main__":
    unittest.main()
